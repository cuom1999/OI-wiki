...
định của thư viện chuẩn Python. Nó cũng được dùng rộng rãi trong các môi trường
lập trình khác; chẳng hạn Java SE 7 dùng Timsort để sắp xếp mảng đối tượng không
nguyên thủy.
...
1.  **Nhận diện đoạn đã có thứ tự**: quét mảng cần sắp xếp và nhận diện các dãy
    con liên tiếp đã có thứ tự.
2.  **Mở rộng đoạn đã có thứ tự**: nếu đoạn nhận diện được có độ dài nhỏ hơn
    `MIN_RUN`, dùng sắp xếp chèn để mở rộng đoạn đó.
...
-   **Đoạn tăng dần**: nếu phần tử sau lớn hơn hoặc bằng phần tử trước, tiếp tục
    mở rộng đoạn.
-   **Đoạn giảm dần**: nếu phần tử sau nhỏ hơn phần tử trước, tiếp tục mở rộng
    đoạn, sau đó đảo ngược đoạn này thành tăng dần.
...
-   Nếu đoạn nhận diện được có độ dài lớn hơn hoặc bằng `MIN_RUN`, không cần
    thao tác bổ sung, trực tiếp đẩy đoạn vào ngăn xếp.
...
2.  **Bộ đệm tạm thời**: Các thuật toán hợp nhất tại chỗ truyền thống có hiệu
    ...
    dùng một bộ đệm tạm thời, sao chép đoạn ngắn hơn vào bộ đệm, rồi lần lượt
...
Bằng cách này, Timsort có thể bỏ qua nhiều phép so sánh không cần thiết, xử lý
nhanh các phần tử liên tiếp nhỏ hơn (hoặc lớn hơn) ở một phía và chuyển hàng
loạt chúng vào kết quả hợp nhất.
...
???+ note "Cài đặt bằng mã giả"
    ...
    8 & \qquad \qquad \text{dùng sắp xếp chèn để mở rộng đoạn cần xử lý}\\
    ...
    12 & \qquad \textbf{gọi } \text{mergeCollapse(ngăn xếp)} \ \text{để kiểm tra và hợp nhất các đoạn} \\
    ...
    16 & \textbf{gọi } \text{mergeForceCollapse(ngăn xếp)} \ \text{để hợp nhất lần cuối tất cả đoạn} \\
    \end{array}
    $$
