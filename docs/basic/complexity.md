...
Ký hiệu $\Theta$ đồng thời cho cận trên và cận dưới của một hàm. Nếu chỉ biết cận trên tiệm cận của một hàm mà chưa biết cận dưới tiệm cận của nó, có thể dùng ký hiệu $O$. $f(n)=O(g(n))$ khi và chỉ khi $\exists c,n_0$ sao cho $\forall n \ge n_0,0\le f(n)\le c\cdot g(n)$.
...
Tương tự, ký hiệu $\Omega$ được dùng để mô tả cận dưới tiệm cận của một hàm. $f(n)=\Omega(g(n))$ khi và chỉ khi $\exists c,n_0$ sao cho $\forall n \ge n_0,0\le c\cdot g(n)\le f(n)$.
...
??? note "Chứng minh"
    ...
    Tiếp tục truy hồi qua từng tầng, có thể viết cây truy hồi như sau: ![](./images/master-theorem-proof.svg)
...
Bài viết này chủ yếu giới thiệu độ phức tạp từ góc nhìn phân tích thuật toán. Nếu quan tâm, có thể tìm hiểu sâu hơn tại [Độ phức tạp tính toán](../misc/cc-basic.md).
