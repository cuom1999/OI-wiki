...
Dùng sắp xếp để xử lý dữ liệu giúp hiểu rõ đặc điểm của dữ liệu, đồng thời hỗ trợ việc phân tích và trực quan hóa sau đó. Trong đời sống, chẳng hạn như từ điển hay thực đơn, nếu không được sắp theo một thứ tự nhất định, thời gian để mọi người tìm thấy thứ mình cần sẽ tăng đáng kể.
...
???+ note "Ví dụ: kiểm tra xem dãy đã cho có phần tử bằng nhau hay không"
    ...
    Có thể sắp xếp dãy số này trước. Sau đó nhận thấy rằng nếu có hai số bằng nhau, chúng nhất định nằm ở hai vị trí kề nhau trong dãy mới. Khi đó, quét dãy mới một lần trong $O(n)$ là đủ.
