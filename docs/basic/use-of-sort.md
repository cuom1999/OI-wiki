...
trợ các bước phân tích và trực quan hóa sau đó. Trong đời sống, chẳng hạn như
từ điển hay thực đơn, nếu dữ liệu không được sắp theo một thứ tự nhất định, thời
gian để mọi người tìm thấy thứ mình cần sẽ tăng đáng kể.

Máy tính thường cần xử lý dữ liệu quy mô lớn. Sau khi sắp xếp, ta có thể dựa
trên đặc điểm và nhu cầu của dữ liệu để thiết kế quy trình xử lý tiếp theo.
...
việc chỉ tốn chi phí sắp xếp một lần là đáng giá, vì mỗi lần phân tích sau đó
...
???+ note "Ví dụ: kiểm tra xem dãy đã cho có phần tử bằng nhau hay không"
    ...
    nhau, chúng nhất định nằm ở hai vị trí kề nhau trong dãy đã sắp xếp. Khi đó,
    quét dãy đã sắp xếp một lần trong $O(n)$ là đủ.
