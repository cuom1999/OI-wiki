...
???+ note "Ví dụ: kiểm tra xem dãy đã cho có phần tử bằng nhau hay không"
    ...
    Có thể sắp xếp dãy số này trước. Sau đó nhận thấy rằng nếu có hai số bằng nhau, chúng nhất định nằm ở hai vị trí kề nhau trong dãy mới. Khi đó, chỉ cần quét dãy mới một lần trong $O(n)$.
