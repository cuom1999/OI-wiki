...
tử áp chót của mảng, rồi tiếp tục duy trì tính chất của phần đống còn lại.
...
Khi đó, với nút có chỉ số `i` trong mảng, nút cha, nút con trái và nút con phải
tương ứng là:
...
Tương tự sắp xếp chọn, do có thao tác hoán đổi vị trí, đây là một thuật toán sắp
xếp không ổn định.

### Độ phức tạp thời gian

Độ phức tạp thời gian trong trường hợp tốt nhất, trung bình và bất lợi nhất của
sắp xếp vun đống đều là $O(n\log n)$.
...
=== "C++"
    ...
    void sift_down(int arr[], int start, int end) {
      ...
      while (child <= end) {
        // Chỉ so sánh khi chỉ số nút con còn nằm trong phạm vi.
        ...
        if (arr[parent] >= arr[child])
          ...
          // Ngược lại, hoán đổi cha con rồi tiếp tục so sánh với tầng dưới.
    ...
    void heap_sort(int arr[], int len) {
      ...
      // Hoán đổi phần tử đầu với vị trí ngay trước phần đã sắp xếp,
      // rồi điều chỉnh lại phần còn lại.
