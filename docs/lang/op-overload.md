...
Có hai cách nạp chồng toán tử: nạp chồng thành hàm thành viên hoặc thành hàm
...
Toán tử tăng giảm được chia thành hai loại: tiền tố (`++a`) và hậu tố (`a++`).
Để phân biệt dạng tiền tố và hậu tố, khi nạp chồng dạng hậu tố cần thêm một tham
số hình thức kiểu `int`; tham số này chỉ dùng để phân biệt chữ ký hàm.
...
Với các toán tử tăng giảm có sẵn, toán tử tiền tố trả về tham chiếu, còn toán tử
hậu tố trả về giá trị cũ trước khi tăng/giảm. Dù toán tử sau khi nạp chồng không
...
??? note "Về toán tử so sánh ba chiều trong C++20"
    ...
    `std::weak_ordering` hoặc `std::partial_ordering`), hoặc trả về một đối
    tượng sao cho:
