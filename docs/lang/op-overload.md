Nạp chồng toán tử là việc định nghĩa cách một toán tử hoạt động với các kiểu dữ liệu cụ thể. Đây là một trường hợp đặc biệt của nạp chồng hàm.

Khi một toán tử xuất hiện trong biểu thức và ít nhất một toán hạng của nó có kiểu lớp hoặc kiểu liệt kê, C++ sẽ dùng cơ chế phân giải nạp chồng (overload resolution) để quyết định hàm `operator` nào cần được gọi.[^ref1]
...
???+ note "Một số toán tử có thể nạp chồng"
    ...
    Khác: `()` (gọi hàm); `""` (literal do người dùng định nghĩa[^ref2], từ C++11); `new` (cấp phát bộ nhớ); `,` (toán tử dấu phẩy); `<=>` (so sánh ba chiều[^ref3], từ C++20), v.v.
...
Có hai cách nạp chồng toán tử: nạp chồng thành hàm thành viên hoặc thành hàm không phải thành viên.

Khi nạp chồng thành hàm thành viên, đối tượng hiện tại được truyền ngầm qua con trỏ `this` và đóng vai trò một toán hạng của phép toán; với toán tử hai ngôi, đó là toán hạng bên trái. Vì vậy số tham số viết ra trong hàm sẽ ít hơn số toán hạng của phép toán một đơn vị.
...
Toán tử tăng giảm được chia thành hai loại: tiền tố (`++a`) và hậu tố (`a++`). Để phân biệt dạng tiền tố và hậu tố, khi nạp chồng dạng hậu tố cần thêm một tham số hình thức kiểu `int`; tham số này chỉ dùng để phân biệt chữ ký hàm.
...
Bên dưới định nghĩa một cấu trúc so sánh để cài đặt thứ tự ưu tiên tùy biến cho hàng đợi ưu tiên.
...
??? note "Về toán tử so sánh ba chiều trong C++20"
    ...
    Cũng có thể sử dụng so sánh ba chiều tùy biến. Lúc này cần chọn loại quan hệ thứ tự mà phép so sánh biểu diễn (`std::strong_ordering`, `std::weak_ordering` hoặc `std::partial_ordering`), hoặc trả về một đối tượng sao cho:
