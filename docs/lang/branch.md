...
Câu lệnh `if` đánh giá điều kiện; nếu điều kiện đúng, các câu lệnh trong thân sẽ
được thực thi, nếu không thì bị bỏ qua.
...
điều kiện. Khi điều kiện của `if` thỏa mãn, thân của `if` sẽ được thực thi; khi
điều kiện không thỏa mãn, thân của `else` sẽ được thực thi. Tương tự, nếu phần
thân chỉ có một câu lệnh, có thể lược bỏ cặp dấu ngoặc nhọn.
...
Câu lệnh `else if` kết hợp `if` và `else`, dùng để xét nhiều điều kiện và chọn
một trong nhiều nhánh câu lệnh. Câu lệnh `else` cuối cùng không cần viết thêm
điều kiện. Ví dụ, nếu `dieu_kien1` đúng thì thực thi `than1`; nếu `dieu_kien3`
đúng còn `dieu_kien1` và `dieu_kien2` đều sai thì thực thi `than3`; chỉ khi tất
cả điều kiện đều sai mới thực thi `than4`.
...
>     phương trình có nghiệm thực kép;
> -   Ngược lại,
...
Khi thực thi câu lệnh `switch`, trước tiên chương trình tính giá trị của biểu
thức chọn, sau đó dựa vào giá trị đó để chọn nhãn tương ứng và bắt đầu thực thi
từ vị trí nhãn ấy. Trong C++, biểu thức chọn thường là biểu thức kiểu số nguyên
hoặc kiểu liệt kê; các kiểu lớp chuyển đổi được sang những kiểu này cũng hợp lệ.
Các nhãn `case` phải là biểu thức hằng có thể chuyển đổi sang kiểu của biểu thức
chọn. Ví dụ:
...
tiếp tục thực thi các câu lệnh trong những `case` kế tiếp và cả `default`. Hiện
tượng này gọi là rơi tiếp (`fallthrough`). Ví dụ:
...
không muốn chương trình chạy tiếp sang các nhánh kế tiếp, cần dùng `break`. Ví
dụ:
...
lệnh ở những nhánh tiếp theo sẽ không được thực thi. Nhánh cuối cùng không bắt
buộc phải có `break`, vì sau đó không còn câu lệnh nào nữa.
...
??? note "Hiểu switch như thế nào"
    ...
    Đây cũng là cơ sở của những kỹ thuật đặc biệt như Duff's Device; bạn có thể
    tham khảo thêm khi cần tìm hiểu sâu hơn.
