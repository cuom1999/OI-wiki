...
thường chọn IDE để viết mã. Trong thi đấu, công cụ phổ biến là
...
Khi đã quen hơn, nhiều người chuyển sang dùng dòng lệnh để biên dịch vì cách
này linh hoạt hơn. Khi đó, người viết không phụ thuộc vào IDE mà có thể dùng
trình soạn thảo văn bản quen thuộc để viết mã.
...
Ghi chú: Chương trình C++ thường vẫn biên dịch và chạy được dù không thêm
`-lm`. Vì các đề NOI/NOIP qua các năm đều đưa `-lm` vào tùy chọn biên dịch
C++, phần này cũng giữ tùy chọn đó.
...
thích với C, đồng thời có nhiều tính năng mới giúp thí sinh làm việc hiệu quả
hơn trong phòng thi. Xem thêm
