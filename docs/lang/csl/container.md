...
-   **Map** (`map`): tập hợp các cặp {khóa, giá trị}, được sắp xếp theo vị từ
    so sánh giữa các khóa.
...
-   **Hàng đợi** (`queue`): cấu trúc vào trước ra trước (FIFO), mặc định bọc
    `deque`.
-   **Hàng đợi ưu tiên** (`priority_queue`): hàng đợi mà thứ tự phần tử được
    quyết định bởi một vị từ tác động lên các giá trị được lưu, mặc định bọc
    `vector`.
...
Các khai báo thường có dạng `containerName<typeName,...> name`, nhưng số lượng
và hình thức tham số template (các tham số trong `<>`) thay đổi theo từng bộ
chứa cụ thể.
...
so sánh từng phần tử, mỗi phần tử của `map` tương đương một `pair<key, value>`;
bộ chứa không thứ tự không hỗ trợ `<`/`>`/`<=`/`>=`.
