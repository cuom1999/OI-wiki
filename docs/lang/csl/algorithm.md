...
-   `inplace_merge`: **gộp tại chỗ thành một dãy có thứ tự** hai phạm vi đã được
    sắp xếp theo toán tử nhỏ hơn: `[first,middle)` và `[middle,last)`.
...
-   `upper_bound`: tìm kiếm nhị phân trong một dãy có thứ tự, trả về bộ lặp trỏ
    ...
    trả về bộ lặp cuối. `upper_bound(v.begin(), v.end(), x)`.
    ...
    ???+ warning "Độ phức tạp thời gian của `lower_bound` và `upper_bound`"
        ...
        $O(\log n)$. Nhưng trong bộ chứa kết hợp như `set`, nếu gọi
...
-   Dùng `lower_bound` tìm phần tử gần $x$ nhất trong mảng có thứ tự $a$. Bài ví
    dụ: [UVa10487 Closest Sums][uva10487]
    ...
    ???+ note "Mã mẫu"
        ...
        // gần x hơn phần tử cuối cùng nhỏ hơn x (a[i - 1]);
        // Ngược lại, a[i - 1] (phần tử cuối cùng trong a nhỏ hơn x) là đáp án
...
[uva10487]: https://onlinejudge.org/external/104/10487.pdf
