...
Java là một ngôn ngữ lập trình được sử dụng rộng rãi, có các đặc điểm
**đa nền tảng**, **hướng đối tượng**, **lập trình tổng quát (generic)**, và được
dùng phổ biến trong phát triển ứng dụng Web cấp doanh nghiệp cũng như ứng dụng di
động.
...
Tương tự C/C++, chương trình Java cần một hàm (trong lập trình hướng đối tượng
gọi là phương thức) làm điểm vào để bắt đầu thực thi.
...
phương thức tương tự, nhưng khi chạy, chỉ một phương thức trong số đó được dùng
làm điểm vào. Phương thức này được chỉ định trong tệp `Manifest` của JAR; trong
các kỳ thi OI thường không cần quan tâm đến phần này.

### Chú thích

Giống C/C++, Java dùng `//` cho chú thích một dòng và `/* */` cho chú thích nhiều
dòng.
...
Từ khóa `final` biểu thị rằng biến chỉ có thể được gán một lần; sau khi đã gán
thì không thể gán lại.
...
-   Chuỗi là một lớp có sẵn của Java.
...
một gói không được có hai lớp trùng tên. Dòng đầu của tệp thường khai báo lớp này
thuộc gói nào. Ví dụ:
...
Dùng từ khóa `import` để nhập lớp không nằm trong gói của lớp hiện tại. Ví dụ,
lớp `Scanner` được dùng bên dưới:

```java
import java.util.Scanner;
```

Nếu muốn nhập toàn bộ lớp trong một gói, có thể thay tên lớp trước dấu chấm phẩy
cuối câu lệnh bằng `*`.
...
class Test {
    public static void main(String[] args) {
        Scanner scan = new Scanner(System.in); // System.in là luồng nhập chuẩn
        int a = scan.nextInt();
    ...
    public static void main(String[] args) {
        switch ( /* biểu thức */ ){
          case /* giá trị 1 */:
              ...
              break; // nếu thiếu break, chương trình sẽ chạy tiếp tuần tự đến break
          case /* giá trị 2 */:
...
mảng hoặc bộ chứa. Có thể xem như biến lặp trong cách trước đã được ẩn đi, với
dạng như sau:
...
nhau thì chương trình mới biên dịch được. Trong OJ, tên lớp hoặc tên tệp thường
được quy định cụ thể trong đề hoặc cấu hình ngôn ngữ.
