...
Ngoài ra, có thể khai báo con trỏ trỏ tới một biến con trỏ khác.
...
Với biến cấu trúc, cách làm cũng tương tự. Nếu muốn truy cập thành viên của cấu
trúc mà con trỏ trỏ tới, trước hết cần giải tham chiếu con trỏ, rồi dùng toán tử
truy cập thành viên `.`. Cách viết ngắn gọn hơn là toán tử "mũi tên" `->`.
...
dừng. Giải tham chiếu con trỏ không hợp lệ, chẳng hạn con trỏ null hoặc con trỏ
...
???+ note "Định nghĩa `NULL` trong ngôn ngữ C"
    ...
    một là biểu thức hằng kiểu số nguyên, một là biểu thức hằng được chuyển đổi
...
???+ note "Vấn đề do `NULL` gây ra trong ngôn ngữ C"
    ...
    con trỏ, nhưng người gọi lại truyền vào một `NULL` được định nghĩa là kiểu số
    nguyên, thì có thể gây ra hành vi không xác định. Nguyên nhân là khi lấy tham
    số biến thiên bên trong hàm, chương trình đọc một giá trị được truyền với
...
muốn sửa dữ liệu bên ngoài nó, hoặc nếu dữ liệu của một cấu trúc/lớp lớn và không
phù hợp để sao chép, có thể truyền địa chỉ của dữ liệu bên ngoài vào hàm. Nhờ đó,
hàm có thể truy cập, thậm chí sửa đổi dữ liệu đó.
...
C++ đưa vào khái niệm tham chiếu; trong nhiều tình huống thường gặp, tham chiếu
dễ dùng hơn con trỏ và cũng khó bị dùng sai hơn. Xem thêm
...
dựng đối tượng trên vùng nhớ đó, rồi trả về địa chỉ của vùng nhớ.
...
???+ note "Khởi tạo bằng danh sách"
    ...
    nghĩa. Ngoài ra, dùng toán tử `{}` giúp hình thức khởi tạo biến thống nhất
    hơn. Xem thêm
...
Cũng có thể dùng toán tử `new[]` để tạo mảng. Khi đó, toán tử `new[]` trả về địa
chỉ đầu của mảng, tức địa chỉ của phần tử đầu tiên; có thể dùng con trỏ có kiểu
tương ứng để lưu địa chỉ này. Khi giải phóng, cần dùng toán tử `delete[]`.
...
???+ note "Cách lưu trữ mảng hai chiều"
    Cách lưu dữ liệu theo "hàng" này được gọi là lưu trữ theo thứ tự hàng trước;
    tương ứng, cũng có thể lưu dữ liệu theo cột trước. Do đặc tính truy cập bộ
    nhớ của máy tính, nhìn chung, truy cập dữ liệu liên tiếp sẽ hiệu quả hơn. Vì
    vậy, cần chọn cách lưu trữ "hàng trước" hoặc "cột trước" theo cách dữ liệu sẽ
    được sử dụng.
...
Ngoài ra, cũng có thể cấp phát và sử dụng bộ nhớ theo mô hình "mảng của các
mảng". Với một mảng lưu nhiều mảng, thực chất đó là một mảng lưu địa chỉ đầu của
nhiều mảng, tức một mảng lưu nhiều biến con trỏ.
...
Mảng hai chiều thu được theo cách này không bảo đảm vùng nhớ liên tiếp.
...
Để gọi một hàm, cần biết kiểu tham số, số lượng tham số và kiểu giá trị trả về
của hàm đó; các thông tin này thường được gọi chung là chữ ký hàm.
