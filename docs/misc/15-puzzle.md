...
15 - 拼图是涉及 [启发式算法](../search/heuristic.md) 建模的经典问题．此问题的常见形式是 [曼哈顿距离](../geometry/distance.md#khoảng-cách-manhattan) 和错位方块的数量计算，二者都是可接受启发（英文：admissible heuristic），即它们永远不会高估剩余的移动次数，这确保了某些搜索算法（例如 [A \* 算法](../search/astar.md)）的最优性．
