...
??? note "Ý tưởng giải"
    ...
    Giả sử đã viết xong `solve(l,r)`, và đã xử lý xong bằng đệ quy `solve(l,mid)` cùng `solve(mid+1,r)`. Bây giờ việc cần làm là thống kê trong các cặp điểm $(i,j)$ thỏa $l \leq i \leq mid$, $mid+1 \leq j \leq r$, có bao nhiêu cặp còn thỏa các ràng buộc $a_{i} \leq a_{j}$, $b_{i} \leq b_{j}$, $c_{i} \leq c_{j}$.
    
    Ràng buộc $a_{i} \leq a_{j}$ không còn mấy tác dụng: vì $i$ nhỏ hơn $mid$, $j$ lớn hơn $mid$, nên chắc chắn $i$ nhỏ hơn $j$; dãy đã được sắp xếp theo $a$, do đó nhất định có $a_{i} \leq a_{j}$. Bây giờ chỉ còn hai ràng buộc: $b_{i} \leq b_{j}$ và $c_{i} \leq c_{j}$. Dựa trên các ràng buộc này, có thể duyệt $j$ và tính xem có bao nhiêu $i$ thỏa điều kiện.
    
    Để tiện duyệt, sắp xếp tất cả các điểm trong $(l,mid)$ và $(mid+1,r)$ theo giá trị $b$ tăng dần. Sau đó lần lượt duyệt từng $j$, đưa tất cả các điểm $i$ có $b_{i} \leq b_{j}$ vào một cấu trúc dữ liệu nào đó, trong lời giải này chọn [cây Fenwick](../ds/fenwick.md). Khi đó chỉ cần truy vấn trong cây Fenwick có bao nhiêu điểm có giá trị $c$ không lớn hơn $c_{j}$, sẽ tính được với điểm $j$ này có bao nhiêu $i$ có thể ghép hợp lệ với nó.
    
    Khi chèn một điểm có giá trị $c$ bằng $x$, cộng một tại vị trí $x$ trong cây Fenwick; còn thao tác truy vấn trong cây Fenwick có bao nhiêu điểm nhỏ hơn $x$ thực chất là tính [tổng tiền tố](../basic/prefix-sum.md). Miễn là đã [rời rạc hóa](../misc/discrete.md) tất cả các giá trị $c$ từ trước, độ phức tạp sẽ đúng.
    ...
    Thông qua quy trình như vậy, thông tin về các cặp điểm loại thứ hai được xử lý xong trong thời gian $O(n\log n)$. Khi đó độ phức tạp thời gian của thuật toán là $T(n)=T(\lfloor \frac{n}{2} \rfloor)+T(\lceil \frac{n}{2} \rceil)+O(n\log n)=O(n\log^2n)$.
...
???+ example "[CQOI2011 Cặp nghịch thế động](https://www.luogu.com.cn/problem/P3157)"
    ...
    Bây giờ cho một hoán vị của $1\sim n$, lần lượt xóa $m$ phần tử theo một thứ tự nào đó. Nhiệm vụ là trước mỗi lần xóa một phần tử, thống kê số cặp nghịch thế của toàn bộ dãy.
...
Chuyển trạng thái trực tiếp có độ phức tạp $O(n^2)$. Sau đây là phần giải thích cách dùng chia để trị CDQ để tối ưu quá trình chuyển trạng thái.

Quan hệ chuyển trạng thái từ $dp_{j}$ sang $dp_{i}$ cũng là một dạng quan hệ giữa các cặp điểm, vì vậy có thể xử lý nó theo cách tương tự như khi dùng chia để trị CDQ để xử lý quan hệ cặp điểm.
...
Thông qua mô phỏng luồng gọi hàm, có thể thấy mỗi khi `solve(l,r)` kết thúc, các giá trị DP trong đoạn $(l,r)$ đều đã được tính xong. Vì mỗi lần thực hiện hàm chuyển trạng thái thì `solve(l,mid)` đã kết thúc, nên mọi quá trình chuyển trạng thái đều hợp lệ, thỏa điều kiện thứ 1.

Trong quá trình vừa rồi, nếu xem cây đệ quy của chia để trị CDQ như một cây đoạn, thì chia để trị CDQ chính là **hàm duyệt trung thứ tự** của cây đoạn này. Vì vậy, quy trình này tương đương với việc xử lý tất cả các giá trị DP theo thứ tự, chỉ là thứ tự chuyển trạng thái đã được tách ra mà thôi, nên thuật toán đúng.
...
??? note "Ý tưởng giải"
    ...
    Có thể thử dùng chia để trị CDQ. Đưa toàn bộ truy vấn và thao tác sửa đổi về ngoại tuyến. Các thao tác này tạo thành một dãy, và có $O(N^2)$ cặp quan hệ sửa đổi - truy vấn. Vẫn dùng quy trình chung của chia để trị CDQ: chia tất cả quan hệ thành ba loại, trong tầng chia để trị này chỉ xử lý các quan hệ sửa đổi - truy vấn vượt qua $mid$, các quan hệ sửa đổi - truy vấn còn lại được giải quyết bằng đệ quy.
    ...
    Quan sát quy trình thuật toán trên, ban đầu chỉ có thể giải bài toán tĩnh cộng hình chữ nhật, tính tổng hình chữ nhật; nhưng chỉ cần dùng chia để trị CDQ, đã có thể giải ngoại tuyến một bài toán động cộng hình chữ nhật, tính tổng hình chữ nhật. Tinh túy của việc chuyển bài toán động thành bài toán tĩnh nằm ở chỗ chia để trị CDQ mỗi lần chỉ xử lý quan hệ sửa đổi và truy vấn vượt qua một điểm nào đó, nhờ vậy chỉ cần xét bài toán đơn giản "mọi truy vấn đều nằm sau các sửa đổi". Chính vì điểm này, chia để trị CDQ được gọi là "công cụ chuyển bài toán động thành bài toán tĩnh".
...
??? note "Ý tưởng giải"
    ...
    -   Nếu tất cả các cạnh có trọng số bị sửa trong cây khung nhỏ nhất đều được gán thành $-\infty$, mà cạnh này xuất hiện trong cây, thì chứng minh được nó nhất định sẽ xuất hiện trong cây khung nhỏ nhất của đoạn $(l,r)$. Như vậy có thể dùng DSU để co các đỉnh tương ứng với những cạnh này, đồng thời cộng trọng số của các cạnh đó vào đáp án.
    ...
    Trước hết, các cạnh đã bị sửa chắc chắn sẽ được thêm vào tập cạnh, số lượng các cạnh này ở mức $O(len)$.
    ...
    Việc chứng minh số đỉnh là bội hằng của $O(len)$ trở nên rất đơn giản. Mỗi lần đệ quy xuống, các cạnh bị co là các cạnh chưa bị sửa xuất hiện trong cây khung với $-\infty$; nghĩ ngược lại, tức là đã cắt bỏ tất cả các cạnh đã bị sửa xuất hiện trong cây khung với $-\infty$. Nhiều nhất chỉ cắt bỏ $len$ cạnh, toàn bộ đồ thị nhiều nhất tách thành $O(len)$ thành phần liên thông, như vậy số đỉnh của đồ thị mới là mức $O(len)$. Do đó đã chứng minh rằng mỗi lần đồ thị dùng để chạy Kruskal đều ở mức $O(len)$, từ đó độ phức tạp thời gian của mỗi tầng đều là $O(n\log n)$.
