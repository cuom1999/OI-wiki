...
Bài này đúng với đặc trưng của việc phối hợp Mo và `bitset` đã nói ở trên. Ta dễ nghĩ đến việc dùng riêng một `bitset` để lưu tất cả trọng số đã xuất hiện trong mỗi đoạn. Đáp án của một nhóm truy vấn bằng tổng độ dài các đoạn trừ đi số phần tử trong hợp của ba tập nhân với $3$.

Tuy nhiên, khi dùng `bitset` trong Mo, ta cũng cần điều chỉnh thuật toán theo đặc tính của `bitset`:

1.  `bitset` không xử lý tốt trường hợp cùng lúc có nhiều trọng số giống nhau. Ta có thể lấy tổng của trọng số sau rời rạc hóa của phần tử hiện tại và số lần xuất hiện của nó trong đoạn hiện tại làm đối tượng được chèn vào `bitset`.
2.  Khi dùng Mo thông thường, ta có thể không chú ý đến thứ tự của 4 cách di chuyển con trỏ. Vì vậy trong quá trình di chuyển, có thể xảy ra trường hợp đầu trái của đoạn nằm bên phải đầu phải, khiến độ dài đoạn âm và số lượng phần tử cũng âm. Trong các tình huống khác điều này thường không ảnh hưởng, nhưng trong bài này phần tử chèn vào `bitset` liên quan đến số lượng phần tử. Do đó ta cần chú ý thứ tự của 4 cách di chuyển con trỏ: đặt các câu lệnh di chuyển con trỏ trái sang trái và con trỏ phải sang phải lên trước, để tránh chèn số âm vào `bitset`.
