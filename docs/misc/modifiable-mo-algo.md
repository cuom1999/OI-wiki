...
Cần nắm được thuật toán Mo thông thường trước khi đọc phần này. Nếu chưa quen, nên đọc trước phần [thuật toán Mo thông thường](./mo-algo.md).

## Đặc điểm

Mo thông thường không xử lý được thao tác sửa đổi.

Có thể mở rộng thuật toán để hỗ trợ sửa đổi, tương tự cách thêm trạng thái trong DP: thêm một chiều **thời gian** để biểu diễn thời điểm của thao tác hiện tại.

Chiều thời gian biểu diễn số lần sửa đổi đã trải qua.

Tức là biến truy vấn $[l,r]$ thành $[l,r,\text{thời gian}]$.

Khi đó tọa độ cũng có thể di chuyển trên chiều thời gian. Nói cách khác, $[l,r,\text{thời gian}]$ có thêm một chiều di chuyển và có thể chuyển thành:
...
Các chuyển trạng thái như vậy cũng là $O(1)$, nhưng khi sắp xếp cần xử lý thêm một khóa.

Có thể sắp xếp thứ tự chuyển trạng thái bằng cách tương tự Mo thông thường và đạt $O(n^{5/3})$.

Thông thường chọn $n^{2/3}$ làm độ dài khối, chia thành $n^{1/3}$ khối. Khóa thứ nhất là khối chứa đầu trái, khóa thứ hai là khối chứa đầu phải, khóa thứ ba là thời gian.
...
???+ note "Độ dài khối tối ưu và phân tích độ phức tạp thời gian"
    ...
    Xét trường hợp không chia khối đầu phải:
    ...
    Tiếp theo là phân tích độ phức tạp thời gian.
    ...
    Tức là khi lấy độ dài khối $\dfrac{n^{2/3}t^{1/3}}{m^{1/3}}$, độ phức tạp thời gian tối ưu là $O\left(n^{2/3}m^{2/3}t^{1/3}\right)$.
...
Nếu không có thao tác 1 (sửa đổi), bài toán có thể giải bằng Mo thông thường.

Tuy nhiên, đề bài còn có sửa đổi tại một điểm, nên cần dùng **Mo có sửa đổi**.
...
-   Sửa đổi tại một điểm là thay số ở một vị trí. Giả sử cần chuyển từ một truy vấn đã trải qua $i$ lần sửa đổi sang một truy vấn đã trải qua $j$ lần sửa đổi, và $i<j$, thì cần áp dụng lần lượt các sửa đổi từ lần thứ $i+1$ đến lần thứ $j$.
-   Nếu $j<i$, thì cần hoàn tác lần lượt các sửa đổi từ lần thứ $i$ về lần thứ $j+1$.

Cách áp dụng một sửa đổi như sau. Giả sử một sửa đổi thay màu ở vị trí $pos$: màu ban đầu tại $pos$ là $a$, sau khi sửa là $b$. Đồng thời giả sử đoạn hiện tại của Mo là $[l,r]$.

-   Áp dụng sửa đổi này: trước hết kiểm tra $pos$ có nằm trong đoạn $[l,r]$ hay không. Nếu có, việc này tương đương xóa màu $a$ khỏi đoạn, thêm màu $b$, rồi đổi màu của phần tử thứ $pos$ trong dãy màu hiện tại thành $b$. Nếu không nằm trong đoạn $[l,r]$, chỉ cần trực tiếp đổi phần tử thứ $pos$ trong dãy màu hiện tại thành $b$.
