...
Giả sử $n=m$. Với bài toán truy vấn đoạn trên dãy, nếu từ đáp án của $[l,r]$ có thể mở rộng trong $O(1)$ để thu được đáp án của $[l-1,r],[l+1,r],[l,r+1],[l,r-1]$ - tức các đoạn kề với $[l,r]$ - thì có thể tính đáp án của tất cả truy vấn trong độ phức tạp $O(n\sqrt{n})$.
...
Tiếp theo là phần cốt lõi của thuật toán Mo. Phần này dùng một cách chứng minh sơ cấp, dễ hiểu để chỉ ra độ phức tạp thời gian là $O(n\sqrt{n})$.

???+ note "Chứng minh"
    Chứng minh: Gọi giá trị lớn nhất của $L$ trong từng khối lần lượt là $\max_1,\max_2,\max_3, \cdots , \max_{\lceil\sqrt{n}\rceil}$.
    
    Từ lần sắp xếp thứ nhất, suy ra $\max_1 \le \max_2 \le \cdots \le \max_{\lceil\sqrt{n}\rceil}$.
    
    Với mỗi khối, thời gian để tính trực tiếp truy vấn đầu tiên là $O(n)$.
    ...
    Cộng độ phức tạp theo $L$ của từng khối lại, thu được:
...
Đặt độ dài khối là $S$. Với một số lượng bất kỳ truy vấn nằm trong cùng một khối, quãng đường di chuyển là $n$; tổng cộng có $\displaystyle \frac{n}{S}$ khối, nên tổng số lần di chuyển là $\displaystyle \frac{n^2}{S}$. Việc di chuyển có thể vượt qua biên khối, vì vậy cần cộng thêm độ phức tạp $mS$. Tổng độ phức tạp là $\displaystyle O\left(\frac{n^2}{S}+mS\right)$. Để giá trị này nhỏ nhất, hai hạng tử cần xấp xỉ bằng nhau; do đó chọn $S=\displaystyle \frac{n}{\sqrt{m}}$ là tối ưu. Khi đó độ phức tạp là $\displaystyle O\left(\frac{n^2}{\displaystyle \frac{n}{\sqrt{m}}}+m\left(\frac{n}{\sqrt{m}}\right)\right)=O(n\sqrt{m})$.

Thực tế, nếu đặt độ dài khối không chính xác, độ phức tạp thời gian của Mo sẽ bị ảnh hưởng đáng kể. Ví dụ, nếu $m$ cùng bậc với $\sqrt n$ nhưng lại đặt nhầm độ dài khối là $\sqrt n$, có thể dựng một bộ dữ liệu khiến độ phức tạp trở thành $O(n \sqrt n)$ thay vì giá trị đúng $O(n^{5/4})$.

Thuật toán Mo trông khá đơn giản, phần lớn vì cách sắp xếp theo khối của nó có vẻ thô. Có thể nghĩ đến việc dùng một cách sắp xếp tinh vi hơn cho mọi đoạn. Một cách là xem mỗi đoạn $[l, r]$ như một điểm $(l, r)$ trên mặt phẳng, dựng cây khung nhỏ nhất theo khoảng cách Manhattan trên tất cả các điểm, rồi mỗi lần chuyển đáp án giữa các truy vấn dọc theo cạnh của cây đó. Cách này trông có vẻ cải thiện được độ phức tạp của thuật toán Mo, nhưng cận trên độ phức tạp của cách sắp xếp truy vấn theo khối đã là tối ưu.
...
Thuật toán Mo còn có một đặc điểm: khi $n$ không đổi, $m$ càng lớn thì chi phí chuyển trung bình cho mỗi truy vấn càng nhỏ. Một số thuật toán ngoại tuyến khác cũng có đặc điểm tương tự, chẳng hạn thuật toán tìm LCA ngoại tuyến cổ điển, nhưng với thuật toán Mo, chi phí chuyển trung bình thay đổi theo $m$ rõ rệt nhất.
...
???+ note "Bài mẫu [Đội tuyển tập huấn quốc gia - Tất của Tiểu Z](https://www.luogu.com.cn/problem/P1494)"
    ...
    Cho một dãy $\{c_i\}$ có độ dài $n$. Có $m$ truy vấn, mỗi truy vấn cho hai số $l,r$. Chọn ngẫu nhiên hai số khác nhau trong các số có chỉ số từ $l$ đến $r$, cần tính xác suất hai số đó bằng nhau.
...
**Lưu ý: do có `++l` và `--r`, vị trí của 4 vòng lặp while dùng để di chuyển đoạn trong đoạn mã dưới đây là rất quan trọng; không được tùy tiện thay đổi quan hệ thứ tự giữa chúng.**
...
??? note "Thảo luận về vị trí của bốn vòng lặp"
    ...
    Vì vậy, nếu tại một thời điểm nào đó xuất hiện $l>r+1$, sẽ tồn tại một phần tử có số lần được thêm là âm. Điều này gây lỗi trong một số bài toán; ví dụ nếu dùng một `set` để duy trì tất cả các số trong đoạn, sẽ phát sinh tình huống cần xóa một phần tử không tồn tại trong `set`.
    
    Bốn vòng lặp while trong mã có tổng cộng $4!=24$ thứ tự sắp xếp. Giả sử vòng lặp đầu tiên thao tác với đầu trái, có $12$ thứ tự sau; $12$ thứ tự còn lại là đối xứng. Bảng dưới đây liệt kê tính đúng đắn của 12 cách viết này và đưa ra phản ví dụ cho các cách sai.
    ...
    Trong toàn bộ 24 thứ tự, chỉ có 6 thứ tự đúng. Trong đó có 2 thứ tự có chứng minh khá dài, nên phần này chỉ đưa ra chứng minh cho 4 thứ tự.
    
    Điểm chung của 4 cách viết đúng này là hai bước đầu mở rộng đoạn trước (`l--` hoặc `r++`), hai bước sau mới thu hẹp đoạn (`l++` hoặc `r--`). Viết như vậy thì hai bước đầu là mở rộng đoạn, có thể giữ $l\le r+1$; sau khi thực hiện xong hai bước đầu, luôn có $l\le l'\le r'\le r$, và hai bước sau chỉ thu hẹp đoạn về $[l',r']$, vẫn giữ được $l\le r+1$. Do đó cách viết này là đúng.
...
=== "Bản viết gọn"
    ```cpp
    // clang-format off
    // Có một chi tiết nhỏ sẽ được trình bày ở phần sau
...
=== "Bản viết rõ"
    ...
    struct node {
      ...
      bool operator<(const node &x) const {
        ...
        // Lưu ý hai dòng dưới không được viết <= hoặc >=, nếu không sẽ lỗi
...
Với bản viết gọn, nếu không có trường hợp đặc biệt `r == x.r`, khi l thuộc cùng một khối lẻ và r bằng nhau, vấn đề trong chi tiết nhỏ ở trên sẽ xuất hiện; có thể kiểm chứng bằng cách mô phỏng thủ công. Với bản viết rõ, nếu viết thành nhỏ hơn hoặc bằng, hoặc lớn hơn hoặc bằng, thì cũng sẽ xuất hiện vấn đề tương tự.
