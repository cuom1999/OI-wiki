...
Thuật toán gộp theo kinh nghiệm là gì?

Gộp theo kinh nghiệm là kỹ thuật tối ưu một số thuật toán bằng cách chọn hướng gộp có lợi dựa trên kích thước hoặc cấu trúc hiện tại.

Ví dụ thường gặp nhất là gộp nhỏ vào lớn trong DSU. Mã như sau:
...
Trong đoạn mã này, với hai tập có kích thước khác nhau, tập nhỏ được gộp vào tập lớn thay vì làm ngược lại.

Vì sao lại làm vậy? Trong DSU, kích thước của tập có thể xem như một đại lượng kiểm soát chiều cao của cây đại diện. Gộp cây nhỏ vào cây lớn giúp thao tác tìm đại diện nhanh hơn.

Cho cây nhỏ trở thành cây con của cây lớn hơn là ý tưởng cốt lõi của gộp theo kinh nghiệm.

## Nội dung thuật toán

Gộp theo kinh nghiệm trên cây, hay DSU on tree, là một thuật toán cho một số bài toán ngoại tuyến trên cây. Nó thường đủ nhanh so với nhiều cách tiếp cận khác, đồng thời dễ hiểu và dễ cài đặt hơn.
...
Với dạng bài này, nhiều lời giải thường dùng các cấu trúc dữ liệu nặng như cây lồng cây. Nếu có thể xử lý ngoại tuyến, ta có một cách đơn giản hơn.
...
Trước hết, tiền xử lý kích thước cây con của mỗi đỉnh và con nặng của nó. Giống như trong phân rã chuỗi nặng, con nặng là đỉnh con có cây con chứa nhiều đỉnh nhất. Quá trình này có thể hoàn thành trong $O(n)$.

Dùng $cnt_i$ để biểu diễn số lần xuất hiện của màu $i$, và $ans_u$ để biểu diễn đáp án của đỉnh $u$.

Khi xử lý một đỉnh $u$, thực hiện theo các bước sau:

1.  Trước tiên xử lý các con nhẹ, tức các con không nặng, của $u$ và tính đáp án của chúng, nhưng **không giữ lại ảnh hưởng của lần duyệt đó lên mảng $cnt$**.
2.  Xử lý con nặng của $u$ và **giữ lại ảnh hưởng của con nặng lên mảng $cnt$**.
...
Bằng cách này, với mỗi đỉnh, cây con nặng được duyệt một lần còn các cây con nhẹ được duyệt hai lần; phần lớn dữ liệu được giữ lại ở nhánh nặng.

Sau khi thực hiện quy trình này, ta thu được đáp án cho cây con của đỉnh đó.

Vì sao không gộp bước đầu tiên và bước thứ ba? Vì mảng $cnt$ không thể được dùng lại theo cách đó; nếu giữ riêng ảnh hưởng của mọi cây con nhẹ, bộ nhớ sẽ quá lớn. Thuật toán cần hoàn thành trong không gian $O(n)$.

Nếu một đỉnh $u$ được duyệt $x$ lần, thì con nặng của nó cũng được duyệt $x$ lần, còn các con nhẹ, nếu có, sẽ được duyệt $2x$ lần.

Lưu ý rằng ngoài nhánh con nặng, sau mỗi lần duyệt cần xóa ảnh hưởng đã thêm vào `cnt`.

## Chứng minh

Định nghĩa cạnh nặng và cạnh nhẹ giống như trong phân rã chuỗi nặng: cạnh nối tới con nặng là cạnh nặng, các cạnh còn lại là cạnh nhẹ. Với một cây có $n$ đỉnh, định nghĩa về con nặng và cạnh nặng có thể xem trong hình dưới đây.

Số cạnh nhẹ trên đường đi từ gốc đến bất kỳ đỉnh nào không vượt quá $\log n$. Giả sử từ gốc đến đỉnh đó có $x$ cạnh nhẹ và kích thước cây con của đỉnh đó là $y$. Cây con của đỉnh con được nối bởi một cạnh nhẹ có kích thước nhỏ hơn một nửa kích thước cây con của cha, vì nếu lớn hơn một nửa thì cạnh đó đã là cạnh nặng. Do đó $y<n/2^x$, suy ra $n>2^x$, nên $x<\log n$.

Ngoài ra, nếu một đỉnh là con nặng của cha nó, thì cây con của nó là lớn nhất trong các cây con cùng cha. Vì vậy, khi tính đáp án, các tổ tiên nối với đỉnh đó bằng toàn cạnh nặng sẽ không duyệt lại nó như một phần của cây con nhẹ. Do đó số lần một đỉnh được duyệt bằng số cạnh nhẹ trên đường từ nó đến gốc cộng thêm $1$ cho chính lần duyệt của nó. Vậy mỗi đỉnh được duyệt nhiều nhất $\log n+1$ lần, và tổng độ phức tạp thời gian là $O(n(\log n+1))=O(n\log n)$. Việc xuất đáp án tốn $O(m)$.
...
Trong phần chứng minh đã nhắc đến việc DSU on tree dùng khái niệm con nặng và con nhẹ trong phân rã chuỗi nặng để tăng tốc quá trình gộp. Vì vậy, cũng có thể trực tiếp dùng thứ tự DFS thu được từ phân rã chuỗi nặng, chuyển đệ quy thành lặp để tiếp tục tối ưu hằng số của DSU on tree.
...
1.  Một số bài mà lời giải chuẩn do người ra đề đặt ra là DSU on tree
    ...
    Vì chỉ xét việc xâu sau khi sắp xếp có thể trở thành xâu đối xứng hay không, một ký tự xuất hiện hai lần tương đương với không xuất hiện trong trạng thái chẵn lẻ. Nói cách khác, đường đi này thỏa mãn điều kiện **nhiều nhất một ký tự xuất hiện số lần lẻ**.

    Cách làm thông thường là DFS từ mỗi đỉnh. Mỗi khi đi tới một đỉnh, duyệt toàn bộ chữ cái để tìm các đường đi mà sau khi XOR với trạng thái hiện tại có nhiều nhất một bit bằng $1$, rồi lấy độ dài lớn nhất. Cách này có độ phức tạp $O(n^2\log n)$, và có thể dùng DSU on tree để tối ưu xuống $O(n\log^2n)$. Cách làm cụ thể có thể tham khảo phần đọc thêm bên dưới.
