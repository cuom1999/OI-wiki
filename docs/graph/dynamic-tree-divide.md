...
Sau khi xây cây phân rã trọng tâm, với mỗi đỉnh $x$ duy trì hai **đống hỗ trợ xóa**. $dist[x]$ lưu thông tin khoảng cách từ mọi đỉnh đen trong thành phần liên thông do đỉnh $x$ đại diện đến $x$. $ch[x]$ biểu diễn thông tin khoảng cách từ các đỉnh đen thuộc tất cả các con của đỉnh $x$ trên cây phân rã trọng tâm và chính nó đến $x$. Do cách tham lam để tìm đáp án trong bài này, và hai đường đi đến từ cùng một cây con không thể ghép thành một đường đi hoàn chỉnh, trong đống này chỉ chèn giá trị của chính nó và giá trị lớn nhất trong mỗi cây con. Tổng của hai giá trị lớn nhất trong $ch[x]$, hoặc tổng của tất cả các giá trị nếu không đủ hai giá trị, chính là đường đi dài nhất có hai đầu đen đi qua đỉnh $x$ khi lấy $x$ làm tâm phân rã. Có thể dùng đống hỗ trợ xóa $ans$ để lưu đáp án của mọi đỉnh, khi đó giá trị lớn nhất trong đống này chính là đáp án cần tìm.

Có thể duy trì các đống hỗ trợ xóa $dist[x],ch[x],ans$ theo định nghĩa ở trên. Khi giá trị trong $dist[x]$ thay đổi, cũng có thể duy trì $ch[x],ans$ trong độ phức tạp thời gian $O(\log n)$.
