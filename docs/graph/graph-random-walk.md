...
???+ note "Bài ví dụ 1 [Những vòng tròn chờ đợi](https://codeforces.com/problemset/problem/963/E)"
...
Xét quá trình khử. Khử các phương trình theo thứ tự từ trên xuống dưới trong hệ tọa độ, và trong cùng một lớp thì từ trái sang phải. Tô các phương trình đã khử màu vàng, các điểm kề với điểm vàng màu xanh lá, còn lại màu đen, như hình dưới:
...
???+ note "Bài ví dụ 2 Giá trị kỳ vọng"
...
Bậc của đa thức đặc trưng của một ma trận cấp $n$ $A$ không vượt quá $n$.
...
Nhận thấy kỳ vọng thời gian đi là $E(t)=\sum_{i\geq0}\Pr[t>i]$. Nếu tính được xác suất sau $i$ bước vẫn chưa kết thúc, thì tổng trên mọi $i ≥ 0$ chính là đáp án.
...
???+ note "Bài ví dụ 3 Frank"
    Cho một đồ thị đơn có hướng liên thông mạnh $G = (V, E)$. Với mọi $1 ≤ s ≤ n$, $1 ≤ t ≤ n$, $s ≠ t$, trả lời câu hỏi sau:
    Ban đầu có một quân cờ được đặt tại $v_s$; mỗi giây, quân cờ chọn đều ngẫu nhiên một cạnh trong các cạnh ra của đỉnh hiện tại rồi đi đến đỉnh mà cạnh đó chỉ tới. Tính kỳ vọng thời gian để đến $v_t$. $3 ≤ n ≤ 400$.
...
Để dễ quan sát, viết phương trình chuyển dưới dạng ma trận. Gọi $P$ là ma trận chuyển của đồ thị này, $F$ là ma trận đáp án, $I$ là ma trận đơn vị cấp $n$, $J$ là ma trận cấp $n$ toàn số $1$, và $G$ là một ma trận cấp $n$ thỏa mãn $G_{i,i} = g_i$, còn các vị trí khác bằng $0$. Khi đó:

$$
F=J-G+PF
$$

Nếu tìm được $G$, chỉ cần giải phương trình:
...
Ý nghĩa thực tế của phân phối dừng khá rõ: nếu tại một thời điểm nào đó, quân cờ ở $v_i$ với xác suất $π_i$, thì tại mọi thời điểm về sau, quân cờ vẫn thỏa mãn phân phối xác suất này. Có thể giải hệ phương trình bằng khử Gauss trong $O(n^3)$ thời gian để tìm $π$. Vậy $π$ có quan hệ gì với $G$?
...
???+ note "Chứng minh"
    ...
    Nhân đồng thời hai vế ở bên trái với $π$, có:
...
Vì vậy, bằng cách đưa vào phân phối dừng, có thể tìm $G$ trong $O(n^3)$ thời gian.

### Giải bài toán ban đầu

Trong quá trình giải phương trình, gặp một vấn đề: $(I - P)$ không đầy hạng, nên không thể giải bằng cách nhân với ma trận nghịch đảo.
...
???+ note "Chứng minh"
    Vì nhân một hàng của ma trận với một hằng số khác 0 không làm thay đổi hạng, nhân hàng thứ $i$ của $(I - P)$ với bậc ra của $v_i$ để nhận được một ma trận mới $L$. Khi đó chỉ cần chứng minh hạng của $L$ là $n - 1$.
    ...
    $L$ bằng ma trận bậc ra của đồ thị $G$ trừ đi ma trận kề của nó. Theo Bổ đề 5.1, định thức của $L$ sau khi xóa hàng thứ $i$ và cột thứ $i$ biểu thị số cây khung có hướng gốc $v_i$.
    ...
    Quay lại bài toán ban đầu, xét việc giải phương trình trong bài toán ban đầu. Để tiện, viết phương trình dưới dạng $AX = B$, trong đó $A$, $B$ đã biết và cần tìm $X$. Vì $A$ không đầy hạng, phương trình có vô số nghiệm; trước hết tìm một nghiệm riêng.
