...
**Đồ thị dây cung**: đồ thị mà mọi chu trình độ dài lớn hơn $3$ đều có dây cung.

**Bổ đề 1**: cỡ clique lớn nhất không vượt quá sắc số: $\omega(G)\le \chi(G)$.
...
Chứng minh: nếu đồ thị dây cung có một đồ thị con cảm sinh không phải đồ thị dây cung, thì trong đồ thị con cảm sinh này tồn tại một chu trình không dây cung có độ dài lớn hơn $3$.
Khi đó dù đồ thị ban đầu có thêm cạnh như thế nào, nó cũng không thể là đồ thị dây cung, mâu thuẫn.

**Bổ đề 4**: mọi đồ thị con cảm sinh của đồ thị dây cung không thể là một chu trình có số đỉnh lớn hơn $3$.

Chứng minh: một chu trình có số đỉnh lớn hơn $3$ không phải là đồ thị dây cung, suy ra trực tiếp từ **Bổ đề 3**.
...
Vì $V_1,V_2$ đều là các thành phần liên thông, nên giữa hai cặp đỉnh $x_1,y_1$ và $x_2,y_2$ tồn tại các đường đi ngắn nhất.
Gọi các đường đi ngắn nhất bên trong $V_1,V_2$ lần lượt là $x-x_1\sim y_1-y,x-x_2\sim y_2-y$.
...
Đặt $L=A\cup I$.
Nếu $L$ là đồ thị đầy đủ, thì $u$ là đỉnh đơn hình.
Nếu không, vì $L$ là đồ thị con cảm sinh của đồ thị ban đầu nên nó cũng là đồ thị dây cung, do đó có hai đỉnh đơn hình không kề nhau.
Vì $I$ là một clique, mọi cặp đỉnh trong $I$ đều kề nhau, nên trong $A$ phải tồn tại một đỉnh đơn hình.
Đỉnh đơn hình này mở rộng ra toàn đồ thị vẫn là đỉnh đơn hình.

Do mỗi bước quy nạp tách đồ thị thành các thành phần liên thông nhỏ hơn, và các phần đó đều thỏa mãn tính chất, nên quy nạp đúng.
...
Lặp lại quá trình trên. Nếu mọi đỉnh đều bị xóa, thì đồ thị ban đầu là đồ thị dây cung và thu được một thứ tự khử hoàn hảo; nếu trên đồ thị không tồn tại đỉnh đơn hình, thì đồ thị ban đầu không phải là đồ thị dây cung.
...
Cần chứng minh rằng với mọi đồ thị dây cung, thứ tự do thuật toán tìm được là một thứ tự khử hoàn hảo, tức với mỗi đỉnh, các đỉnh đứng sau nó trong thứ tự và kề với nó đều kề nhau từng đôi một.

**Bổ đề 9**: xét ba đỉnh $u,v,w$ thỏa mãn $\alpha(u)<\alpha(v)<\alpha(w)$.
Nếu $uw$ kề nhau, còn $vw$ không kề nhau, thì $w$ chỉ đóng góp vào $label$ của $u$, không đóng góp vào $label$ của $v$.
Để $v$ được đưa vào thứ tự trước $u$, cần có một đỉnh $x$ thỏa mãn $\alpha(v)<\alpha(x)$, $vx$ kề nhau và $ux$ không kề nhau.
Nói cách khác, $x$ chỉ đóng góp cho $v$ mà không đóng góp cho $u$.
...
Vì $\alpha(v_1)<\alpha(v_k)<\alpha(v_0)$, đồng thời $v_1v_0$ kề nhau và $v_kv_0$ không kề nhau, nên theo **Bổ đề 9**, tồn tại $x$ thỏa mãn $\alpha(v_k)<\alpha(x)$, $v_kx$ kề nhau và $v_1x$ không kề nhau.

Xét chỉ số nhỏ nhất $j\in(1,k]$ sao cho $v_jx$ kề nhau. Có thể suy ra $v_0x$ không kề nhau; nếu không, $v_0v_1\cdots v_jx$ sẽ tạo thành một chu trình không dây cung có độ dài $\ge 4$.
...
Theo định nghĩa thứ tự khử hoàn hảo, giả sử các đỉnh kề với $v_i$ trong $\{v_i,v_{i+1},\ldots , v_n\}$, sắp theo thứ tự tăng dần, là $\{v_{c_1},v_{c_2},\ldots ,v_{c_k} \}$.
Khi đó chỉ cần kiểm tra $v_{c_1}$ có kề trực tiếp với các đỉnh còn lại hay không.
...
Chứng minh: xét một clique cực đại $V$ của đồ thị dây cung.
Gọi $x$ là đỉnh trong clique này xuất hiện đầu tiên trong thứ tự khử hoàn hảo.
Khi đó $V\subseteq \{x\}\cup N(x)$.
Lại vì $V$ là clique cực đại, nên $V=\{x\}\cup N(x)$.
...
Gọi $nxt_x$ là đỉnh đứng sớm nhất trong thứ tự khử hoàn hảo trong $N(x)$, và gọi $y^*$ là đỉnh đứng muộn nhất trong mọi $y$ thỏa mãn $A\subseteq B$.
Khi đó $nxt_{y^*}=x$.
Nếu không, $y^*$ không phải là đỉnh muộn nhất, vì đặt $y^*=nxt_{y^*}$ vẫn thỏa mãn điều kiện.
...
## Sắc số và cỡ clique lớn nhất của đồ thị dây cung
...
Theo định nghĩa, $t\le \alpha(G),t\ge \kappa(G)$.
Theo **Bổ đề 2**, $\alpha(G)\le \kappa(G)$, nên $t=\alpha(G)=\kappa(G)$.
...
[P3852\[TJOI2007\] Em nhỏ](https://www.luogu.com.cn/problem/P3852)
...
[Simple linear-time algorithms to test chordality of graphs, test acyclicity of hypergraphs, and selectively reduce acyclic hypergraphs](https://dl.acm.org/doi/abs/10.1137/0213035),
