...
Các cài đặt C++ và Python bên dưới dựa trên cách lưu đồ thị bằng sao tiến dạng danh sách liên kết. Có thể tham khảo cách cài đặt này ở trang [Lưu trữ đồ thị](./save.md).
...
Cụ thể, dùng một hàng đợi `Q` để ghi lại các đỉnh cần xử lý, rồi mở một mảng Boolean `vis[]` để đánh dấu một đỉnh đã được thăm hay chưa.

Ban đầu, đặt giá trị `vis` của mọi đỉnh là 0, biểu thị rằng chúng chưa được thăm. Sau đó, đưa đỉnh xuất phát `s` vào hàng đợi `Q` và đặt `vis[s]` bằng 1.

Tiếp theo, mỗi lần lấy đỉnh đầu hàng đợi `Q`, ký hiệu là `u`, rồi đánh dấu tất cả các đỉnh `v` kề với `u` là đã thăm và đưa chúng vào hàng đợi `Q`.

Lặp lại cho đến khi hàng đợi `Q` rỗng, khi đó BFS kết thúc.

Trong quá trình BFS, cũng có thể ghi lại một số thông tin bổ sung. Ví dụ trong đoạn mã trên, mảng `d` dùng để ghi khoảng cách ngắn nhất từ đỉnh xuất phát đến một đỉnh nào đó, tức số cạnh ít nhất cần đi qua, còn mảng `p` ghi lại đỉnh mà từ đó đi đến đỉnh hiện tại.

Với mảng `d`, có thể lấy khoảng cách từ đỉnh xuất phát đến một đỉnh.

Với mảng `p`, có thể khôi phục đường đi ngắn nhất từ đỉnh xuất phát đến một đỉnh. Hàm `restore` trong đoạn mã trên dùng mảng này để lần lượt in ra các đỉnh trên đường đi ngắn nhất từ đỉnh xuất phát đến đỉnh `x`.
...
Khi cài đặt BFS, về bản chất cần đưa các đỉnh chưa được thăm vào một vùng chứa gọi là open, và đưa các đỉnh đã được thăm vào một vùng chứa gọi là closed.
...
Tương tự, cũng có thể định nghĩa cây BFS: trong quá trình BFS, bằng cách ghi lại mỗi đỉnh được thăm từ đỉnh nào, có thể xây dựng một cấu trúc cây, gọi là cây BFS.
...
Nếu chưa biết hàng đợi hai đầu `deque`, có thể tham khảo [phần liên quan đến deque](../lang/csl/sequence-container.md#deque).
...
Trong trường hợp thông thường, đưa đỉnh được mở rộng qua cạnh không có trọng số vào đầu hàng đợi, và đưa đỉnh được mở rộng qua cạnh có trọng số vào cuối hàng đợi. Như vậy có thể bảo đảm rằng, giống như BFS thông thường, trọng số trong toàn bộ hàng đợi sẽ không giảm từ đầu đến cuối.
...
Cho một lưới $n \times m$. Hiện có một tia laser bắn từ góc trên bên trái sang bên phải. Mỗi khi gặp ký tự '#', có thể chọn cho tia sáng bắn ra theo bốn hướng hoặc không làm gì cả. Hỏi cần ít nhất bao nhiêu ký tự '#' bắn ra theo bốn hướng để tia sáng có thể bắn ra bên phải ở hàng thứ $n$.
