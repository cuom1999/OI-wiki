...
// Tính tổng khoảng cách từ đỉnh 1 đến mọi đỉnh khác.
void dfs1(int u, int fa) {
  ...
  for (int v : g[u]) {
    ...
    dfs1(v, u);
    siz[u] += siz[v];
    dp[u] += dp[v] + siz[v];  // Tổng khoảng cách từ các đỉnh trong cây con đến u.
  }
}

// Dùng DP đổi gốc để tính tổng khoảng cách khi mỗi đỉnh làm gốc.
