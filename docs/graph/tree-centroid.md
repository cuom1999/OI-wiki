...
???+ info "Cây con"
    Bài viết này có thể đồng thời đề cập đến cây không gốc, cây có gốc, và cây thu được khi đổi gốc của cây có gốc sang một đỉnh không phải gốc. Để tránh nhầm lẫn, bài viết dùng $T$ để biểu diễn cây không gốc, và dùng $T^{(v)}$ để biểu diễn cây có gốc tại đỉnh $v$. Trong bài viết này, "cây con" đều chỉ cây gồm một đỉnh và tất cả các đỉnh hậu duệ của nó trong **cây có gốc**. Trong cây có gốc $T^{(v)}$, cây con tương ứng với đỉnh $u$ được ký hiệu là $T^{(v)}_u$. Cây con được định nghĩa như vậy bao gồm cả bản thân cây ban đầu. Nếu cần nhấn mạnh rằng không bao gồm toàn bộ cây, có thể gọi là "cây con thực sự".
    
    Trong cây không gốc, "cây con" thường chỉ một đồ thị con liên thông của nó. Khi thảo luận về trọng tâm, một số tác giả dùng từ "cây con" riêng để chỉ đồ thị con liên thông cực đại không chứa một đỉnh nào đó, hoặc riêng để chỉ một trong hai thành phần liên thông thu được sau khi xóa một cạnh nào đó. Có thể thấy rằng hai cách định nghĩa này cho ra cùng một tập "cây con", và tập này không bao gồm chính cả cây. Vì tập đó không trùng với tập các cây con của cây có gốc, bài viết này sẽ tránh dùng khái niệm "cây con" cho cây không gốc.
    
    Khi thực tế tìm trọng tâm hoặc xử lý một số bài toán, thường chọn ngầm một gốc cho cây. Khi đó, trong các thành phần liên thông thu được sau khi xóa một đỉnh không phải gốc $v$, ngoài các cây con tương ứng với các đỉnh con của đỉnh đó, còn có một cây con "hướng lên". Nếu cha của đỉnh $v$ là $u$, thì cây con "hướng lên" này chính là $T_u^{(v)}$. Khi đề cập đến loại đồ thị con này, bài viết gọi là cây con "hướng lên". Nếu không có ghi chú đặc biệt, các cây con được đề cập trong bài viết đều không bao gồm loại cây con "hướng lên" này.

Lưu ý rằng các thành phần liên thông thu được cũng là cây không gốc. Bằng cách xóa trọng tâm của cây, cây được tách thành nhiều cây có kích thước nhiều nhất bằng một nửa cây ban đầu. Tính chất này của trọng tâm giúp có thể áp dụng kỹ thuật chia để trị trên cây. Đây là [phân trị theo đỉnh](./tree-divide.md#phân-trị-theo-đỉnh), còn được gọi là phân rã trọng tâm của cây.
...
??? note "Chứng minh"
    Trước hết, đưa vào một số ký hiệu. Hai cách phát biểu cho cây có gốc và cây không gốc là tương đương. Định nghĩa $W(x)=\max_{u\sim x}|T_u^{(x)}|$, trong đó $u\sim x$ ký hiệu $u$ kề với $x$. Định nghĩa $S(x)=\sum_{u\in T}d(u,x)$, trong đó $d(u,x)$ ký hiệu khoảng cách giữa đỉnh $u$ và $x$. Khi đó, định nghĩa 1 tương đương với yêu cầu $W(v)\le |T|/2$, định nghĩa 2 tương đương với yêu cầu $v\in\arg\min_{x\in T}W(x)$, và định nghĩa 3 tương đương với yêu cầu $v\in\arg\min_{x\in T}S(x)$. Cần chứng minh ba điều kiện này là tương đương.
    
    Xem $S(x)$ là tổng độ sâu của các đỉnh khi lấy $x$ làm gốc, và xét thay đổi của nó khi đổi gốc từ đỉnh $v$ sang đỉnh kề $u$. Lưu ý rằng sau khi xóa cạnh $(v,u)$ trong cây, hai thành phần liên thông thu được lần lượt là các cây con $T_v^{(u)}$ và $T_u^{(v)}$. Trước và sau khi đổi gốc, độ sâu của mỗi đỉnh trong cây con $T_v^{(u)}$ tăng $1$, còn độ sâu của mỗi đỉnh trong cây con $T_u^{(v)}$ giảm $1$, nên thay đổi của tổng độ sâu là
    
    $$
    \Delta S_{v\to u} = S(u) - S(v) = |T_v^{(u)}| - |T_u^{(v)}| = |T| - 2|T_u^{(v)}|.
    $$
    
    Vì vậy, điều kiện của định nghĩa 1 tương đương với yêu cầu $\Delta S_{v\to u}\ge 0$ với mọi đỉnh $u$ kề với $v$, tức là $v$ là một điểm cực tiểu của $S(x)$.
    
    Ngược lại, giả sử $v$ là một điểm đạt giá trị nhỏ nhất của $S(x)$ (tức định nghĩa 3). Điểm đạt min tồn tại và là một điểm cực tiểu. Xét cây có gốc $T^{(v)}$ lấy $v$ làm gốc. Đặt $u\neq v$ là một đỉnh không phải gốc; trên đường đi có hướng từ $v$ đến $u$, đỉnh ngay sau $v$ là $y$ (có thể chính là $u$), và đỉnh ngay trước $u$ là $x$ (có thể chính là $v$). Khi đó, vì $T_u^{(x)}\subseteq T_y^{(v)}$, có
    
    $$
    2|T_x^{(u)}| = 2|T| - 2|T_u^{(x)}| \ge 2|T| - 2|T_y^{(v)}| \ge |T|.
    $$
    
    Ở bước cuối, sử dụng việc $v$ là điểm cực tiểu của $S(x)$. Lúc này có hai trường hợp:
    
    -   Tồn tại đỉnh $u$ sao cho $2|T_x^{(u)}|=|T|$. Khi đó, theo bất đẳng thức trên, nhất định có $(x,u)=(v,y)$ và $|T_v^{(u)}|=|T_{u}^{(v)}| = |T|/2$. Tức là đỉnh $u$ đạt dấu bằng chỉ có thể là duy nhất, và nó nhất định kề với $v$. Khi đó, với mọi đỉnh khác $u'\neq u,v$, nhất định tồn tại $x'\sim u'$ sao cho $|T_{x'}^{(u')}| > |T|/2$.
        ...
        Lại vì $\Delta S_{v\to u} = 0$, nên $S(v)=S(u)$. Vì $v$ là điểm đạt giá trị nhỏ nhất, $u$ cũng là điểm đạt giá trị nhỏ nhất. Còn với mọi $u'\neq u,v$, đều tồn tại $x'\sim u'$ sao cho $|T_{x'}^{(u')}| > |T|/2$, điều này vi phạm điều kiện mà một điểm cực tiểu cần thỏa mãn, nên $u'$ cũng không thể là điểm đạt giá trị nhỏ nhất. Do đó, tập các đỉnh thỏa điều kiện 3 là $\arg\min S(x) = \{v,u\}$.
    -   Không tồn tại đỉnh $u$ sao cho $2|T_x^{(u)}|=|T|$. Khi đó, với mọi đỉnh $u\neq v$, đều tồn tại đỉnh $x\sim u$ sao cho $|T_x^{(u)}| > |T|/2$. Lặp lại phân tích ở trên, với mọi đỉnh $u\neq v$, có $W(u) > |T|/2$, và $u$ không phải điểm cực tiểu của $S(x)$. Do đó, chỉ có $v$ thỏa điều kiện 1, và $\arg\min W(x)=\arg\min S(x) = \{v\}$.
...
???+ note "Tính chất"
    ...
    2.  Nếu thêm một lá vào cây hoặc xóa một lá khỏi cây, thì trọng tâm của cây di chuyển nhiều nhất một cạnh.
    ...
    4.  Trọng tâm của cây có gốc luôn nằm trên chuỗi nặng chứa đỉnh gốc. Trọng tâm của toàn cây là tổ tiên của trọng tâm của cây con tương ứng với con nặng của gốc cây đó.
...
??? note "Chứng minh"
    ...
    Từ hai trường hợp trên, có thể thấy trọng tâm của cây mới nhất định nằm trên đường đi từ trọng tâm của cây cũ đến đỉnh lá mới thêm.
    
    Tính chất 3 có thể được chứng minh bằng quy nạp. Giả sử khi nối $T$ và $T'$, cạnh mới thêm là $(x,y)$, với $x\in T,y\in T'$. Không mất tính tổng quát, giả sử một trọng tâm của cây mới nằm trong $T$. Xét quá trình bắt đầu từ cây $T$ và lần lượt thêm các đỉnh trong cây $T'$ vào làm các đỉnh lá. Có thể chứng minh bằng quy nạp rằng một trọng tâm của cây luôn nằm trên đường đi nối trọng tâm của cây $T$ với đỉnh $x$. Trường hợp cơ sở suy ra trực tiếp. Giả sử mệnh đề vẫn đúng cho đến một thời điểm nào đó, và khi đó trọng tâm là $v$. Theo phân tích của tính chất 2, trọng tâm của cây mới nhất định nằm trên đường đi nối đỉnh mới thêm và trọng tâm hiện tại $v$. Hơn nữa, vì nó sẽ không di chuyển ra ngoài cây $T$, chỉ cần xét phần giao của đường đi đó với cây $T$, tức đường đi nối trọng tâm hiện tại $v$ và đỉnh $x$. Theo giả thiết quy nạp, $v$ nằm trên đường đi nối trọng tâm của cây $T$ và đỉnh $x$, nên trọng tâm của cây mới cũng nhất định nằm trên đường đi nối trọng tâm hiện tại $v$ và đỉnh $x$. Theo quy nạp, mệnh đề được chứng minh.
    
    Với tính chất 4, chỉ cần kết hợp với [tính chất của phân rã nặng-nhẹ](./hld.md#tính-chất-của-phân-rã-chuỗi-nặng). Giả sử một trọng tâm của cây $T$ là $v$. Khi đỉnh $v$ là đỉnh gốc, mệnh đề đúng trực tiếp. Xét trường hợp $v$ không phải đỉnh gốc, và $u$ là cha của nó. Vì $|T_u^{(v)}| \le |T|/2$, kích thước cây con chứa $v$ ít nhất là $|T|/2$. Nếu đường đi từ $v$ đến gốc đi qua một cạnh nhẹ, kích thước cây con chứa $v$ sẽ nhỏ hơn hẳn $|T|/2$, mâu thuẫn. Vì vậy, $v$ nhất định nằm trên chuỗi nặng chứa đỉnh gốc. Tiếp theo, theo định nghĩa của chuỗi nặng, trong cây con tương ứng với con nặng của đỉnh gốc, chuỗi nặng chứa đỉnh gốc là một phần của chuỗi nặng chứa đỉnh gốc trong cây ban đầu. Hơn nữa, theo tính chất 3, sau khi thêm con nặng và tất cả các cây con tương ứng với các con nhẹ của nó vào cây con tương ứng với con nặng, vị trí trọng tâm sẽ di chuyển dọc theo đường đi giữa trọng tâm hiện tại và đỉnh gốc, nên trọng tâm mới nhất định là tổ tiên của trọng tâm cũ.

## Cách tìm

Theo các định nghĩa tương đương của trọng tâm, có hai cách tìm tất cả các trọng tâm của cây trong thời gian $O(n)$, trong đó $n$ là số đỉnh của cây.

### DFS thống kê kích thước cây con

Có thể dùng DFS để tính kích thước của mỗi cây con. Với mỗi đỉnh, ghi lại kích thước các cây con tương ứng với tất cả các đỉnh con của nó, đồng thời lấy tổng số đỉnh trừ đi kích thước cây con hiện tại để thu được kích thước của cây con "hướng lên". Sau đó có thể tìm trọng tâm theo định nghĩa.
...
Một cách khác là dùng DP đổi gốc để tính tổng độ sâu của tất cả các đỉnh khi lấy từng đỉnh khác nhau làm gốc (tức tổng khoảng cách đến gốc hiện tại). Theo định nghĩa, chỉ cần tìm đỉnh làm cho tổng độ sâu này nhỏ nhất.
...
??? note "Ý tưởng lời giải"
    ...
    Tương tự cách tìm trọng tâm bằng DFS đã nêu ở trên, với mỗi cây con có gốc tại đỉnh $u$, trước tiên tìm trọng tâm của tất cả các cây con có gốc tại các đỉnh con trực tiếp của nó (trọng tâm của một đỉnh lá là chính nó), rồi lần ngược lên để kiểm tra các đỉnh trên đường đi có phải trọng tâm hay không.
