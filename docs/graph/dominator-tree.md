...
std::vector<int> ord;     // Thứ tự hậu tố ngược của đồ thị
std::bitset<N> dom[N];
std::vector<int> Dom[N];
...
Cây thống trị của một đồ thị dạng cây chính là bản thân nó.
...
```cpp
std::stack<int> sta;
std::vector<int> e[N], g[N], tree[N];  // g là đồ thị đảo của đồ thị gốc, tree là cây thống trị
int n, s, in[N], tpn[N], dep[N], idom[N];  // n là tổng số đỉnh, s là đỉnh bắt đầu, in là bậc vào
int fth[N][17];
...
Trước hết, xuất phát từ $s$ và chạy dfs trên đồ thị có hướng này. Các đỉnh và cạnh đi qua tạo thành một cây $T$. Các cạnh đã đi qua được gọi là cạnh cây, các cạnh còn lại là cạnh không thuộc cây; đặt $dfn(u)$ là thứ tự đỉnh $u$ được duyệt tới; định nghĩa $u<v$ khi và chỉ khi $dfn(u) < dfn(v)$.
...
Bán thống trị có một số tính chất hữu ích:

**Bổ đề 7:** Với mọi đỉnh $u$, $sdom(u) < u$.

**Chứng minh:** Theo định nghĩa, cha $fa(u)$ của $u$ trên $T$ cũng thỏa mãn điều kiện trở thành bán thống trị, và $fa(u) < u$, vì vậy mọi đỉnh lớn hơn $u$ đều không thể trở thành bán thống trị của nó.
...
Xét việc chứng minh $v_0 \rightarrow \dots \rightarrow v_j$ là một đường đi thỏa mãn điều kiện trở thành bán thống trị của $v_j$, tức là chứng minh $\forall i \in [1, j), v_i>v_j$. Nếu không, chọn $i$ sao cho $v_i<v_j$ và $v_i$ nhỏ nhất. Khi đó mọi đỉnh trên đường đi $v_i\rightarrow\dots\rightarrow v_j$ đều không nhỏ hơn $v_i$. Theo tính chất của cây DFS, $v_i$ phải là tổ tiên của $v_j$, và do đó cũng là tổ tiên của $v_{k-1}$; điều này mâu thuẫn với tính tối thiểu của $j$. Vì vậy $sdom(v_j)\le sdom(u)$. Lại có $x\le sdom(v_j)$, nên $sdom(u)\ge x$. Kết hợp với $sdom(u)\le x$ đã chứng minh ở trên, suy ra $x=sdom(u)$.

Theo định lý 1, có thể tìm bán thống trị của mỗi đỉnh. Nút thắt độ phức tạp khi tính bán thống trị nằm ở trường hợp thứ hai, nên xét tối ưu bằng DSU có trọng số, cập nhật giá trị nhỏ nhất mỗi khi nén đường đi.
...
Từ hai định lý trên, thu được quan hệ giữa $sdom(u)$ và $idom(u)$.
