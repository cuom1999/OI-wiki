...
Một ví dụ điển hình của ghép cặp trên đồ thị hai phía trong đời sống là ghép đôi nam nữ. Giả sử có một số nam sinh ($X$) và nữ sinh ($Y$), mỗi người chỉ có thể được ghép cặp một lần, và các cặp được phép ghép đã được giới hạn bởi một danh sách nào đó ($E$). Khi đó, nhiệm vụ của thuật toán ghép cặp lớn nhất trên đồ thị hai phía là tìm số cặp ghép nhiều nhất dưới các ràng buộc này, sao cho có nhiều người được ghép cặp nhất có thể.

???+ info "Gợi ý"
    Bài viết này giả sử ta đã biết một cách chia (tô màu) tập đỉnh $V$ của đồ thị hai phía: $V=X\cup Y$. Nếu ban đầu chưa biết cách chia tập đỉnh $V$ của đồ thị hai phía, có thể dùng [thuật toán tô màu đồ thị hai phía](../bi-graph.md#kiểm-tra) để tìm một cách chia như vậy trong thời gian $O(|V|+|E|)$.
...
Để tìm ghép cặp lớn nhất, thuật toán lần lượt xét tất cả các đỉnh, tìm một đường tăng xuất phát từ đỉnh đó rồi tăng ghép cặp theo đường này. Vì độ dài của đường tăng luôn là số lẻ, nên trong đồ thị hai phía, hai đầu mút của nó nhất định nằm ở hai phía trái và phải. Điều này cho thấy chỉ cần xét các đường tăng xuất phát từ phía trái.

Để tìm đường tăng, có thể định hướng đồ thị hai phía dựa trên ghép cặp hiện tại $M$. Trong một đường tăng xuất phát từ một đỉnh chưa ghép ở phía trái (hoặc bất kỳ đường luân phiên nào), ta chỉ có thể đi theo cạnh không thuộc ghép cặp từ đỉnh phía trái sang đỉnh phía phải, rồi đi theo cạnh thuộc ghép cặp từ đỉnh phía phải về đỉnh phía trái. Vì vậy, có thể quy định mọi cạnh không thuộc ghép cặp đều hướng về phía phải, còn mọi cạnh thuộc ghép cặp đều hướng về phía trái. Bài toán tìm đường tăng được chuyển thành bài toán tìm một đường đi đơn trong đồ thị có hướng, xuất phát từ một đỉnh chưa ghép ở phía trái và đi tới một đỉnh chưa ghép nào đó. Bài toán này có thể giải dễ dàng bằng [DFS](../dfs.md) hoặc [BFS](../bfs.md) trong thời gian $O(|E|)$.

![](images/bigraph-match-1.svg)

(Trong hình, đỉnh màu đậm là đỉnh đã ghép, đỉnh màu nhạt là đỉnh chưa ghép, cạnh đỏ là cạnh thuộc ghép cặp, cạnh đen là cạnh không thuộc ghép cặp, mũi tên biểu thị hướng tương ứng với ghép cặp hiện tại. Từ hình có thể thấy đường $1\rightarrow 8\rightarrow 3\rightarrow 11\rightarrow 6\rightarrow 12$ là một đường tăng đối với ghép cặp hiện tại.)

Khi thuật toán bắt đầu, mọi cạnh đều hướng về phía phải. Sau mỗi lần tìm được đường tăng, cần đảo hướng tất cả các cạnh trên đường tăng đó để biểu thị rằng trạng thái ghép cặp của chúng đã được lật. Khi thuật toán kết thúc, mọi cạnh hướng về phía trái chính là các cạnh thuộc ghép cặp.
...
1.  Thuật toán Kuhn dựa trên bổ đề Berge, mà bổ đề này không yêu cầu phải cho trước hai phía trái và phải của đồ thị hai phía. Vì vậy, ngay cả khi hai phía chưa được chia rõ ràng, thuật toán Kuhn vẫn chạy đúng miễn là bản thân đồ thị là đồ thị hai phía. Tuy nhiên, việc tô màu đồ thị hai phía trước để xác định phía trái và phía phải thường hiệu quả hơn.
2.  Vì độ phức tạp thời gian của thuật toán Kuhn mô tả ở trên thực ra là $O(|X||E|)$, ta có thể chọn phía nhỏ hơn trong hai phía của đồ thị hai phía làm phía trái $X$.
3.  Khi tìm đường tăng, không cần xóa dấu đánh dấu dùng để tránh tìm lặp lại sau mỗi lần DFS. Có thể thử tìm đường tăng cho tất cả các đỉnh chưa ghép ở phía trái trước khi xóa dấu. Trong một lượt tìm như vậy, mỗi cạnh được thăm nhiều nhất một lần, nên độ phức tạp vẫn là $O(|E|)$; nhưng một lượt có thể tìm được nhiều đường tăng, do đó tổng số lượt $k$ không vượt quá $|M|+1$, trong đó $M$ là ghép cặp lớn nhất. Tương ứng, độ phức tạp toàn bộ thuật toán giảm xuống $O(k|E|)$.
4.  Khi tìm đường tăng, ưu tiên xét các đỉnh chưa ghép ở phía phải, vì điều này thường tạo ra đường tăng ngắn hơn.
5.  Vì bổ đề Berge không yêu cầu ghép cặp ban đầu phải rỗng, khi bắt đầu thuật toán Kuhn có thể chọn ngẫu nhiên một số cạnh đôi một không chung đỉnh làm ghép cặp ban đầu để giảm số lần tìm kiếm về sau. Nếu đã áp dụng tối ưu 3, có thể bỏ qua tối ưu này.

Mặc dù độ phức tạp xấu nhất vẫn là $O(|V||E|)$, thuật toán Kuhn được tối ưu đầy đủ vẫn có hiệu quả không tệ. Tuy nhiên, để tránh một số dữ liệu đặc biệt đẩy thuật toán tới độ phức tạp xấu nhất, trước khi ghép cặp cần xáo trộn ngẫu nhiên thứ tự cạnh hoặc thứ tự đỉnh.
...
Thuật toán Hopcroft-Karp tiếp tục tối ưu quá trình tìm đường tăng của thuật toán Kuhn, giảm tổng số lượt xuống $O(|V|^{1/2})$, nhờ đó đạt độ phức tạp thời gian $O(|V|^{1/2}|E|)$. Thuật toán này thực chất là một trường hợp đặc biệt của [thuật toán Dinic](../flow/max-flow.md#thuật-toán-dinic).
...
3.  Lần lượt xuất phát từ từng đỉnh chưa ghép ở phía trái và chạy DFS để tìm đường tăng rồi tăng ghép cặp. Khi DFS, chỉ mở rộng theo các cạnh thỏa mãn tầng liên tiếp và tăng nghiêm ngặt (tức $d(v') = d(v) + 1$), đồng thời chỉ thăm các đỉnh chưa được thăm trong lượt DFS hiện tại. Đặc biệt, DFS sẽ không thăm các đỉnh mà bước BFS trước đó chưa thăm.

Theo thuật ngữ luồng mạng, bước 2 xây dựng một đồ thị phân tầng, còn bước 3 tìm một luồng chặn trên đồ thị phân tầng đó. Đồ thị phân tầng là đồ thị mà mỗi cạnh nhất định đi từ tầng hiện tại sang tầng kế tiếp; còn luồng chặn, trong ngữ cảnh này, là một nhóm cực đại các đường tăng đôi một không có đỉnh chung. Nhóm đường tăng thu được ở bước 3 nhất định là cực đại: nếu không, giả sử tồn tại một đường tăng mới, thì khi thuật toán xét tới đỉnh đầu của đường đó, lẽ ra nó đã tìm được đường này.
...
Khi xây dựng đồ thị phân tầng, giống như thuật toán Dinic thông thường, thuật toán Hopcroft-Karp dừng lại ngay khi tới được một đỉnh chưa ghép ở phía phải. Tuy nhiên, riêng với bài toán ghép cặp trên đồ thị hai phía, điều này không cần thiết. Hơn nữa, vì BFS dừng quá sớm và giới hạn phạm vi DFS về sau, số đường tăng tìm được trong mỗi lượt bị hạn chế, khiến hiệu quả ghép cặp tổng thể chậm lại. Trên một số đồ thị, nó thậm chí còn kém hiệu quả hơn thuật toán Kuhn đã tối ưu. Vì vậy, một cải tiến đơn giản là không dừng BFS sớm, mà xây dựng đồ thị phân tầng cho mọi đỉnh có thể tới được.

??? note "Chứng minh tính đúng đắn"
    Trong thuật toán đã tối ưu, các đường tăng trong luồng chặn không còn có cùng độ dài, nên chứng minh độ phức tạp ở phần trước cũng không còn áp dụng trực tiếp. Tuy nhiên, có thể chứng minh rằng bằng cách xây dựng một đồ thị phụ cho mỗi lượt của thuật toán, ta vẫn thiết lập được kết luận độ dài đường tăng ngắn nhất tăng nghiêm ngặt, từ đó bảo đảm độ phức tạp xấu nhất vẫn đúng.
    
    Giả sử đồ thị hai phía là $G=(X,Y,E)$ và ghép cặp hiện tại là $M$. Gọi $W\subseteq Y$ là tập các đỉnh chưa ghép ở phía phải có thể được BFS thăm tới, và độ dài đường tăng ngắn nhất tới $y\in W$ là $d(y)$. Đặt $d_\text{max} = \max_{y\in W}d(y)$. Khi đó, với mỗi $y\in W$, ta tạo thêm một chuỗi bắt đầu từ $y$ có độ dài $d_\text{max} - d(y)$, đánh dấu lần lượt các đỉnh mới là đỉnh phía trái và đỉnh phía phải, đồng thời đánh dấu lần lượt các cạnh mới là cạnh thuộc ghép cặp và cạnh không thuộc ghép cặp. Gọi đồ thị thu được là $G'=(X',Y',E')$, ghép cặp là $M'$, và độ dài đường tăng ngắn nhất là $d_\text{max}$. Khi đó, các đường tăng đối với $M$ trong đồ thị $G$ mà có thể tìm được theo đồ thị phân tầng, tức các đường tăng ngắn nhất tới đỉnh tương ứng, song ánh với các đường tăng ngắn nhất toàn cục đối với $M'$ trong đồ thị $G'$. Vì vậy, việc tìm luồng chặn trong đồ thị phân tầng của $G$ rồi tăng ghép cặp tương đương với việc tìm luồng chặn trong đồ thị phân tầng của $G'$ rồi tăng ghép cặp. Theo chứng minh ở trên, sau khi tăng, trong đồ thị $G'$ sẽ không còn đường tăng nào có độ dài $d_\text{max}$. Do đó, trong đồ thị $G$ cũng không còn đường tăng nào có độ dài $d_\text{min}=\min_{y\in W} d(y)$: bởi một đường tăng như vậy, sau khi nối thêm đường luân phiên mới, nhất định tương ứng với một đường tăng có độ dài $d_\text{max}$ trong $G'$. Như vậy ta lại thu được kết luận rằng giữa các lượt khác nhau của thuật toán, độ dài đường tăng ngắn nhất tăng nghiêm ngặt. Vì thế, độ phức tạp tổng thể vẫn là $O(|M|^{1/2}|E|)$.
...
Bất kỳ thuật toán nào giải được bài toán luồng cực đại đều có thể dùng để giải bài toán ghép cặp lớn nhất trên đồ thị hai phía. Dễ thấy thuật toán Kuhn và thuật toán Hopcroft-Karp đều là các trường hợp đặc biệt của những thuật toán tương ứng trong bài toán luồng cực đại. Tương tự, [thuật toán đẩy tiền luồng](../flow/max-flow.md#thuật-toán-đẩy-tiền-luồng-push-relabel) cũng có thể dùng để giải bài toán ghép cặp lớn nhất trên đồ thị hai phía. Tuy nhiên, cần chú ý rằng khi áp dụng bất kỳ thuật toán luồng cực đại nào cho bài toán ghép cặp lớn nhất trên đồ thị hai phía, đều cần tối ưu có mục tiêu để tránh hằng số quá lớn.

### Dạng quy hoạch tuyến tính

Giống như các bài toán luồng cực đại khác, bài toán ghép cặp lớn nhất trên đồ thị hai phía $G=(V,E)$ có thể viết thành một bài toán quy hoạch tuyến tính. Nếu dùng $x_e\in\{0,1\}$ để biểu thị cạnh $e$ có thuộc ghép cặp hay không, ta nhận được bài toán quy hoạch tuyến tính sau:
...
Trong đó, $e\sim v$ biểu thị quan hệ liên thuộc, tức đỉnh $v$ là một trong hai đầu mút của cạnh $e$. Ngoài ràng buộc không âm, các ràng buộc của bài toán còn yêu cầu tại mỗi đỉnh $v\in V$ có nhiều nhất một cạnh liên thuộc, đúng với định nghĩa của ghép cặp. Vì vậy, mọi ghép cặp đều tương ứng với một số điểm nguyên trong miền khả thi của quy hoạch tuyến tính này.
...
-   Tập đỉnh đạt được theo chẵn $\mathcal E$, tức tập tất cả các đỉnh có thể tới được từ một đỉnh chưa ghép bằng một đường luân phiên có độ dài chẵn;
-   Tập đỉnh đạt được theo lẻ $\mathcal O$, tức tập tất cả các đỉnh có thể tới được từ một đỉnh chưa ghép bằng một đường luân phiên có độ dài lẻ;
-   Tập đỉnh không đạt được $\mathcal U$, tức tập tất cả các đỉnh không thể tới được từ một đỉnh chưa ghép bằng đường luân phiên.
...
??? note "Chứng minh"
    1.  Theo định nghĩa, $\mathcal U$ rời với $\mathcal E\cup\mathcal O$. Ta chỉ cần chứng minh $\mathcal E$ và $\mathcal O$ rời nhau. Giả sử ngược lại, với một đỉnh $v\in\mathcal E\cap\mathcal O$, tồn tại một đường luân phiên độ dài chẵn từ đỉnh chưa ghép $a$ tới $v$, đồng thời tồn tại một đường luân phiên độ dài lẻ từ đỉnh chưa ghép $b$ tới $v$. Vì đồ thị $G$ là đồ thị hai phía, $a\neq b$, và hai đường đi tới $v$ bằng các cạnh lần lượt là cạnh thuộc ghép cặp và cạnh không thuộc ghép cặp. Do đó, nối hai đường này lại sẽ thu được một đường luân phiên từ $a$ qua $v$ tới $b$. Đây là một đường tăng. Điều này mâu thuẫn với việc $M$ là ghép cặp lớn nhất. Vì vậy, $\mathcal E\cap\mathcal O=\varnothing$.
    
        Gọi $M'$ là một ghép cặp lớn nhất khác với $M$. Lặp lại [chứng minh bổ đề Berge](./graph-match.md#bổ-đề-berge), có thể chỉ ra rằng $M'\oplus M$ chỉ gồm các đường đi độ dài chẵn và các chu trình chẵn. Xuất phát từ ghép cặp lớn nhất $M$, nếu lần lượt lật các cạnh trong những thành phần liên thông này (đường đi và chu trình), tức hoán đổi cạnh thuộc ghép cặp với cạnh không thuộc ghép cặp, ta có thể nhận được ghép cặp lớn nhất $M'$. Khi lật một chu trình chẵn, các đỉnh chưa ghép vẫn là chưa ghép, và tính chẵn lẻ của độ dài đường luân phiên xuất phát từ chúng cũng không đổi; khi lật một đường đi độ dài chẵn, trạng thái ghép cặp của hai đầu mút hoán đổi, nhưng tính chẵn lẻ của độ dài đường đi từ chúng tới bất kỳ đỉnh nào trên đường cũng nhất quán. Vì vậy, trong quá trình lật, các tập $\mathcal E,\mathcal O,\mathcal U$ luôn giữ nguyên. Điều này chứng tỏ phân rã không phụ thuộc vào cách chọn ghép cặp lớn nhất $M$.
...
??? note "Chứng minh"
    ...
    Tiếp theo, cần chứng minh $C$ là phủ đỉnh nhỏ nhất. Để phủ tất cả các cạnh của ghép cặp lớn nhất $M$, bất kỳ phủ đỉnh nào cũng cần ít nhất $|M|$ đỉnh. Vì vậy, chỉ cần chứng minh $|C|=|M|$, thì $C$ chắc chắn là phủ đỉnh nhỏ nhất. Điều này tương đương với việc chứng minh rằng ngoài việc chứa đúng một đầu mút của mỗi cạnh thuộc ghép cặp, $C$ không chứa đỉnh nào khác; nói cách khác, $C$ không chứa đỉnh chưa ghép. Giả sử ngược lại, tồn tại đỉnh chưa ghép $v\in C$. Nếu $v\in X$, thì nhất định $v\in U\subseteq Z$, mâu thuẫn với cách xây dựng $C$; nếu $v\in Y$, thì một đường luân phiên đi tới $v$ là một đường tăng đối với $M$, mâu thuẫn với việc $M$ là ghép cặp lớn nhất theo bổ đề Berge. Các mâu thuẫn này cho thấy không tồn tại đỉnh chưa ghép như vậy, do đó $C$ là phủ đỉnh nhỏ nhất.
...
Bài toán phủ đường đi nhỏ nhất yêu cầu chọn số lượng đường đi đơn ít nhất trong một đồ thị có hướng sao cho mọi đỉnh đều xuất hiện đúng trong một đường đi.
...
??? note "Chứng minh"
    Mỗi ghép cặp $M'$ của đồ thị hai phía $G'$ tương ứng với một đồ thị con $F$ của đồ thị $G$, và trong đồ thị con $F$, bậc vào và bậc ra của mỗi đỉnh đều không vượt quá một. Nói cách khác, đồ thị con $F$ thực chất là một tập các đường đi hoặc chu trình rời nhau trong đồ thị có hướng $G$. Tuy nhiên, ta đã giả sử $G$ không chứa chu trình, nên $F$ chỉ chứa một số đường đi rời nhau. Ngược lại, với mỗi đồ thị con $F$ như vậy, ta đều có thể xây dựng ghép cặp tương ứng. Vì kích thước của ghép cặp $M$ chính là hiệu giữa số đỉnh và số đường đi trong $F$, nên bài toán phủ đường đi nhỏ nhất của đồ thị $G$ tương ứng với bài toán ghép cặp lớn nhất của đồ thị $G'$.
...
??? note "Lời giải"
    Nhận thấy rằng nếu tồn tại $n$ số $1$ sao cho chúng không cùng hàng và không cùng cột với nhau, thì chắc chắn có lời giải; ngược lại chắc chắn vô nghiệm. Bài toán chuyển thành việc có thể tìm được $n$ số $1$ như vậy hay không.
    
    Xét một số $1$. Trong phương án cuối cùng, việc chọn số $1$ này biểu thị rằng hàng và cột của nó đã bị chiếm. Vì vậy, ta xây dựng một đồ thị hai phía có $n$ đỉnh phía trái và $n$ đỉnh phía phải; với mỗi phần tử bằng $1$, thêm một cạnh nối đỉnh phía trái đại diện cho hàng của nó với đỉnh phía phải đại diện cho cột của nó. Sau đó chỉ cần chạy ghép cặp trên đồ thị hai phía.
...
??? note "Lời giải"
    ...
    Rõ ràng, một lần gộp theo hàng và một lần gộp theo cột không được can thiệp vào cùng một viên gạch; đồng thời số lần gộp càng nhiều thì số viên gạch càng ít. Vì vậy, có thể lấy các lần gộp theo hàng làm đỉnh phía trái, các lần gộp theo cột làm đỉnh phía phải, và lấy các xung đột ở trên làm cạnh để xây dựng một đồ thị hai phía. Khi đó, bài toán ban đầu trở thành bài toán tập độc lập lớn nhất trên đồ thị hai phía.
...
??? note "Lời giải"
    ...
    Bây giờ thêm lại thao tác xóa phần tử, ta thấy rất khó xử lý: xóa một cạnh có thể gây biến đổi lớn trong ghép cặp, khiến độ phức tạp không chấp nhận được. Vì vậy, tốt hơn là làm ngược lại: mỗi lần thêm một cạnh, rồi tăng ghép cặp lại theo thứ tự. Do đó bài này chỉ có thể dùng thuật toán Kuhn.
...
-   [Luogu P5030 - Đặt hươu cao cổ cổ dài](https://www.luogu.com.cn/problem/P5030)
