...
(Ở đây xét đồ thị vô hướng không có khuyên.)
...
Với mọi đồ thị G, ta có $\chi(G) \leq \Delta(G) + 1$, trong đó $\Delta(G)$ là bậc cực đại.
...
???+ note "Chứng minh"
    ...
    Trước hết, khi $n\leq 3$, mệnh đề hiển nhiên đúng.
    
    Tiếp theo, giả sử mệnh đề đã đúng với $n-1$. Ta sẽ từng bước tăng cường mệnh đề cần chứng minh.
    ...
    Với một đồ thị chính quy G bất kỳ không phải đồ thị đầy đủ cũng không phải chu trình lẻ, chọn tùy ý một đỉnh v và xét đồ thị con $H:=G-v$. Theo giả thiết quy nạp, $\chi(H)\leq\Delta(H)=\Delta(G)$. Do đó ta chỉ cần chứng minh rằng chèn lại v vào H không làm thay đổi kết luận.
    ...
    Tiếp theo, xét đồ thị con $H_{i,j}$ tạo bởi tất cả các đỉnh trong H được tô màu $c_i$ hoặc $c_j$ cùng với tất cả các cạnh giữa chúng. Không mất tính tổng quát, giả sử với hai đỉnh khác nhau bất kỳ $v_i$, $v_j$, chúng luôn nằm trong cùng một thành phần liên thông của $H_{i,j}$. Nếu chúng nằm ở hai thành phần liên thông khác nhau, ta có thể hoán đổi màu của tất cả các đỉnh trong một thành phần liên thông, khiến $v_i$ và $v_j$ có cùng màu.
    
    > Ở đây, hoán đổi màu nghĩa là: nếu trong đồ thị chỉ có hai màu a và b, thì đổi tất cả các đỉnh đang có màu a thành màu b, đồng thời đổi tất cả các đỉnh đang có màu b thành màu a.
    
    Gọi thành phần liên thông nói trên là $C_{i,j}$. Khi đó $C_{i,j}$ nhất định chỉ có thể là một đường đi từ $v_i$ đến $v_j$. Vì bậc của $v_i$ trong H là $\Delta-1$, các đỉnh kề của $v_i$ trong H phải có màu đôi một khác nhau; nếu không ta có thể tô lại $v_i$ bằng một màu khác, làm màu của nó trùng với một đỉnh kề khác của v. Do đó số đỉnh kề của $v_i$ trong $C_{i,j}$ là 1, và với $v_j$ cũng tương tự. Tiếp theo, lấy một đường đi từ $v_i$ đến $v_j$ trong $C_{i,j}$ và gọi nó là P. Nếu $C_{i,j}\ne P$, ta tô lại màu các đỉnh trên đường đi P theo thứ tự. Gọi u là đỉnh đầu tiên gặp được có bậc lớn hơn 2. Chú ý rằng các đỉnh kề của u dùng nhiều nhất $\Delta-2$ màu, nên có thể tô lại u, từ đó làm cho $v_i$ và $v_j$ không còn liên thông với nhau.
    
    Khi đó không khó thấy rằng với ba đỉnh khác nhau bất kỳ $v_i$, $v_j$, $v_k$, ta có $V(C_{i,j})\cap V(C_{j,k})=\{v_j\}$.
...
Trước hết, sắp xếp các đỉnh theo bậc giảm dần, ta được:
...
???+ note "Chứng minh"
    ...
    Đặt $V_0=\varnothing$. Ta lấy tập con $V_m$ của $V(G)\setminus\bigcup_{i=0}^{m-1} V_i$, trong đó các phần tử thỏa mãn:
    ...
    Rõ ràng, nếu tô các đỉnh trong $V_i$ bằng màu thứ i, phương án tô màu này chính là phương án do thuật toán Welsh-Powell đưa ra. Ta cũng có ngay:
    ...
    Bất đẳng thức bên trái hiển nhiên đúng. Ta xét bất đẳng thức bên phải.
    
    Trước hết, không khó suy ra rằng:
    ...
    Mặt khác, dựa trên cách xây dựng dãy $\{V_i\}$, không khó thấy rằng
    
    $v_j\in\bigcup_{i=1}^j V_i$
    
    Kết hợp hai công thức trên, ta được điều phải chứng minh.
...
???+ note "Chứng minh"
    ...
    Khi thử thêm cạnh $(x,y)$, ta tìm màu chưa được dùng có số hiệu nhỏ nhất tại x và tại y, giả sử lần lượt là $l_x$ và $l_y$.
    
    Nếu $l_x=l_y$, ta có thể trực tiếp đặt màu của cạnh này là $l_x$.
    
    Ngược lại, giả sử $l_x<l_y$, ta có thể thử đổi màu cạnh đi ra từ đỉnh $y$ đang có màu $l_x$ thành $l_y$.
    ...
    Vì đường tăng là hữu hạn, ta có thể đảo màu tất cả các cạnh trên đường tăng: cạnh vốn có màu $l_x$ được đổi thành $l_y$, còn cạnh vốn có màu $l_y$ được đổi thành $l_x$.
    ...
    Vì vậy, sau khi tăng, ta có thể trực tiếp đặt màu của cạnh nối $x$ và $y$ là $l_x$.
...
??? note "Một bài ví dụ khá khó [uoj 444 Đồ thị hai phía](https://uoj.ac/problem/444)"
    ...
    Trước hết, ta có thể thấy cận dưới của đáp án là số đỉnh có bậc không phải bội của k.
    
    Cách xây dựng đạt cận dưới là tách đỉnh trên đồ thị hai phía.
    
    Nếu $degree \bmod k \neq 0$, ta tách nó thành $degree/k$ đỉnh có bậc k và một đỉnh có bậc $degree \bmod k$.
    
    Nếu $degree \bmod k = 0$, ta tách nó thành $degree/k$ đỉnh có bậc k.
    ...
    Theo định lý Vizing, ta hiển nhiên có thể xây dựng một phương án k-tô màu cho đồ thị này.
    
    Phần xóa cạnh không liên quan nhiều đến định lý Vizing, nên không trình bày thêm ở đây.
    
    Bạn đọc quan tâm có thể tự đọc lời giải mà tác giả đã viết khi đó.
