...
Trong hệ tọa độ vuông góc, một robot xuất phát từ một điểm bất kỳ và thực hiện $\textit{n}$ lần di chuyển; mỗi lần đi sang phải $\textit{dx}$ và đi lên $\textit{dy}$. Cuối cùng đường đi tạo thành một đa giác đơn khép kín trên mặt phẳng. Hãy tính số điểm trên biên, số điểm bên trong đa giác và diện tích đa giác.

### Lời giải

Bài này thực chất dùng ba kiến thức sau:

-   Với một đoạn thẳng có hai đầu mút là điểm nguyên, nếu $\textit{dx}$ và $\textit{dy}$ của cạnh đều khác $0$, số điểm lưới mà đoạn đi qua là $\gcd(\textit{dx}, \textit{dy}) + 1$. Dĩ nhiên, khi tính cho cả hình, điểm cộng thêm sẽ được cạnh trước đó tính rồi, nên không cần cộng nữa. Vì vậy số điểm mà một cạnh đóng góp là $\gcd(\textit{dx},\textit{dy})$, trong đó $\textit{dx},\textit{dy}$ lần lượt là độ lệch theo phương ngang và phương dọc của đoạn thẳng. Nếu $\textit{dx}$ hoặc $\textit{dy}$ bằng $0$, số điểm đóng góp là $\textit{dy}$ **hoặc** $\textit{dx}$.
