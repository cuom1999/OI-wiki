...
Cây Descartes ở trên tương ứng với việc xem giá trị phần tử của mảng là khóa $w$, còn chỉ số mảng là khóa $k$. Có thể thấy khóa $k$ của cây này thỏa tính chất BST, còn khóa $w$ thỏa tính chất min-heap. Đồng thời, theo tính chất của cây tìm kiếm nhị phân, ta thấy cây Descartes đặc biệt này có đặc điểm: các chỉ số nằm trong một cây con tạo thành một đoạn liên tiếp.

Khi dùng cây Descartes trong thi đấu, ta thường lấy chỉ số mảng làm khóa $k$ trong cặp khóa-giá trị, và chỉ số mảng $k$ thỏa tính chất BST.

Trong phần sau, khi dùng $k,w$, mặc định $k$ thỏa tính chất BST và $w$ thỏa tính chất heap.
...
Với một cây Descartes, định nghĩa "chuỗi phải" là chuỗi thu được bằng cách bắt đầu từ nút gốc rồi liên tục đi theo con phải cho đến một nút lá. Sau khi chèn một nút, nút đó chắc chắn nằm trên chuỗi phải. Vì ta chèn theo thứ tự tăng dần của $k$, vốn là khóa thỏa tính chất BST, nên nút mới chèn chắc chắn nằm ở **đầu mút phải** của cây. Nút này không thể là con trái và cũng không có con phải.
...
??? note "Ý tưởng giải"
    Cụ thể, ta dùng chỉ số làm khóa $k$, dùng $h_i$ làm khóa $w$ thỏa tính chất min-heap, rồi dựng một cây Descartes của các cặp $(i,h_i)$.

    Khi đó, ta duyệt từng nút $u$ và lấy $w_u$ (tức chiều cao $h$ của nút $u$) làm chiều cao của hình chữ nhật con lớn nhất tương ứng. Vì cây Descartes đã dựng thỏa tính chất min-heap, mọi nút trong cây con của $u$ đều có chiều cao lớn hơn hoặc bằng $u$. Mặt khác, ta đã biết các chỉ số trong cây con của $u$ tạo thành một đoạn liên tiếp. Vì vậy, chỉ cần biết kích thước cây con là có thể tính diện tích hình chữ nhật con lớn nhất của đoạn này. Dùng giá trị tính được từ từng điểm để cập nhật đáp án. Rõ ràng việc này có thể hoàn thành bằng một lần DFS, nên độ phức tạp là $O(n)$.
