...
Hợp nhất và tách cây phân đoạn là những kỹ thuật thường dùng với cây phân đoạn, phổ biến trong các bài toán dùng cây
phân đoạn theo giá trị để duy trì đa tập.

Chẳng hạn, nếu một số đỉnh của cây có các thao tác riêng, thông tin cần được truyền từ đỉnh con lên đỉnh cha theo
hướng từ dưới lên, và thông tin tại từng đỉnh lại thuận tiện để duy trì bằng cây phân đoạn, thì có thể dùng kỹ thuật
hợp nhất cây phân đoạn để kiểm soát độ phức tạp tổng thể.

## Hợp nhất cây phân đoạn

### Quy trình

Đúng như tên gọi, hợp nhất cây phân đoạn là xây dựng một cây phân đoạn mới, trong đó mỗi nút là kết quả sau khi hợp
nhất hai nút tương ứng của hai cây phân đoạn ban đầu. Kỹ thuật này thường được dùng để duy trì thông tin trên cây hoặc
trên đồ thị.

Dĩ nhiên, không thể thật sự xây đầy đủ một cây phân đoạn mới mỗi lần hợp nhất, vì vậy cần dùng cây phân đoạn mở nút
động đã trình bày ở phần trước.
...
???+ note "Độ phức tạp của hợp nhất cây phân đoạn"
    ...
    nút tăng thêm cuối cùng xấp xỉ cấp $n\log n$. Do đó, độ phức tạp tổng để hợp nhất tất cả các cây phân đoạn là cấp
...
Lưu ý rằng khi cả tách và hợp nhất cùng tồn tại, cần thu hồi nút trong lúc hợp nhất, để tránh khả năng một nút bị
chiếm dụng lặp lại khi tách.

Để tách đoạn $[l,r]$ từ một cây phân đoạn có miền $[1,N]$ và xây một cây mới:

Bắt đầu tách đệ quy từ nút số 1. Khi nút không tồn tại, hoặc đoạn $[s,t]$ mà nút đại diện không giao với $[l,r]$,
quay lui trực tiếp.
