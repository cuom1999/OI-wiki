...
Việc duy trì các tính chất này khá phức tạp. Khi chèn một nút, trước hết nút đó phải được tô màu đỏ, nếu không sẽ phá vỡ
tính chất 4. Nhưng ngay cả như vậy, thao tác chèn vẫn có thể phá vỡ tính chất 3, nên cần thực hiện điều chỉnh. Xóa nút
còn rắc rối hơn: tương tự như khi chèn, không thể xóa trực tiếp một nút đen, nếu không cân bằng chiều cao đen sẽ bị phá
vỡ. Vấn đề là cần một cách thuận tiện để xử lý các trường hợp này.
...
Vì vậy cần thực hiện một phép xoay phải. Với trạng thái sau phép xoay phải, nên gọi `color_flip`, tức là đảo màu của nút
đó và hai con của nó.
...
??? note "Mã tham khảo (một phần)"
    ...
    typename Set<Key, Compare>::Node *Set<Key, Compare>::fix_up(
        Set::Node *root) const {
      if (is_red(root->rc) && !is_red(root->lc))  // Sửa cạnh đỏ nghiêng phải
        root = rotate_left(root);
      if (is_red(root->lc) &&
          is_red(root->lc->lc))  // Sửa hai cạnh đỏ nghiêng trái liên tiếp
        // Nếu root->lc == nullptr thì biểu thức thứ hai sẽ không được tính
...
trái sẽ **tạm thời** bị phá vỡ, nhưng khi quay lui khỏi đệ quy thì chúng sẽ được khôi phục.

Như hình dưới mô tả, đây là một trường hợp tương đối đơn giản. Lúc này `h->rc->lc` là màu đen, chỉ cần đảo màu một lần:
...
??? note "Mã tham khảo (một phần)"
    ...
    typename Set<Key, Compare>::Node *Set<Key, Compare>::delete_min(
      ...
      if (!is_red(root->lc) && !is_red(root->lc->lc)) {
        // Bảo đảm root->lc hoặc root->lc->lc là đỏ,
        // từ đó bảo đảm cuối cùng sẽ xóa một nút đỏ.
...
Vậy nếu không có cây con phải thì sao? Cần xoay cây con trái lên, nhờ đó vấn đề này sẽ không xuất hiện.
...
??? note "Mã tham khảo (một phần)"
    ...
    typename Set<Key, Compare>::Node *Set<Key, Compare>::delete_arbitrary(
      ...
      if (cmp_(key, root->key)) {
        ...
        // root->lc sau khi đi sâu vào hàm) là đỏ, để cuối cùng sẽ xóa
...
Đoạn mã dưới đây là một `Set` được cài đặt bằng cây đỏ-đen nghiêng trái, tức là một tập hợp có thứ tự và không chứa phần
tử trùng lặp:
...
??? note "Mã tham khảo"
    ...
    class Set {
     private:
      ...
      struct Node {
        ...
        size_t size{0};
        NodeColor color;  // Màu của liên kết từ cha
    ...
    const Key &Set<Key, Compare>::get_min(Set::Node *root) const {
      Node *x = root;
      // Chủ động để lỗi nếu root == nullptr
