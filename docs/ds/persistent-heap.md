Heap có thể hợp nhất bền vững thường được dùng trong các bài toán $k$ đường đi ngắn nhất.
...
Nói cách khác, hiệu năng không bị suy giảm bởi dữ liệu đặc biệt.

## Cây leftist bền vững

Trước khi đọc phần này, cần nắm các kiến thức liên quan về [cây leftist](./leftist-tree.md).

### Quy trình

Nhắc lại quá trình hợp nhất cây leftist.
Giả sử cần hợp nhất hai cây leftist có gốc lần lượt là $x,y$,
và cây leftist được duy trì theo tính chất min-heap:
...
Với cây leftist, chỉ cần sao chép đường đi bị sửa đổi trong quá trình thao tác.

Vì vậy, quá trình hợp nhất của cây leftist bền vững như sau:

1.  Nếu một trong hai nút $x,y$ rỗng, trả về $x+y$.

2.  Chọn nút có khóa nhỏ hơn trong hai nút $x,y$, tạo một bản sao mới $p$ của nút đó, rồi dùng $p$ làm gốc của cây leftist sau hợp nhất.
...
Vì một thao tác trên cây leftist chỉ sửa và tạo mới nhiều nhất $O(\log n)$ nút,
nếu số thao tác là $m$ thì độ phức tạp thời gian và không gian của cây leftist bền vững đều là $O(m\log n)$.
