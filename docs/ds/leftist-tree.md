...
**Cây lệch trái**, giống như [**đống ghép cặp**](./pairing-heap.md), là một loại **đống có thể hợp nhất**:
nó vừa thỏa tính chất đống, vừa hỗ trợ hợp nhất nhanh.
...
Về cấu trúc, cây lệch trái là một cây nhị phân. Ngoài tính chất đống, nó còn "lệch trái":
...
Khi hợp nhất hai đống, trước hết chọn gốc có giá trị nhỏ hơn làm gốc của đống kết quả để giữ tính chất đống (bài này xét
đống nhỏ). Sau đó giữ nguyên con trái của gốc này, rồi đệ quy hợp nhất con phải của nó với đống còn lại để tạo ra con
...
???+ note "Cài đặt"
    ...
    int merge(int x, int y) {
      if (!x || !y) return x | y;  // Nếu một đống rỗng thì trả về đống còn lại
      if (t[x].val > t[y].val) swap(x, y);  // Lấy nút có giá trị nhỏ hơn làm gốc
      t[x].rs = merge(t[x].rs, y);          // Đệ quy hợp nhất con phải với đống còn lại
...
Nhờ tính chất lệch trái, mỗi khi đệ quy xuống một tầng, $\mathrm{dist}$ của gốc trong một trong hai đống sẽ giảm $1$.
Mặt khác, trong một cây nhị phân có $n$ nút, $\mathrm{dist}$ của gốc không vượt quá $\left\lceil\log (n+1)\right\rceil$.
Vì vậy, độ phức tạp khi hợp nhất hai đống có kích thước lần lượt là $n$ và $m$ là $O(\log n+\log m)$.
...
Đoạn mã sau viết theo biến thể đống lớn; nếu dùng đống nhỏ thì đổi chiều so sánh giá trị.
...
Một nút đơn lẻ cũng có thể xem là một đống, vì vậy chỉ cần hợp nhất.
...
### Cộng/trừ một giá trị cho toàn bộ đống, nhân với một số dương

Nhìn chung, mọi thao tác có thể gắn đánh dấu lười mà không làm thay đổi thứ tự tương đối đều xử lý được.

Gắn đánh dấu ở gốc; khi xóa gốc hoặc hợp nhất đống, tức khi cần truy cập con, thì đẩy đánh dấu xuống:
...
chứng minh chi tiết có thể xem tại [đống ngẫu nhiên](https://cp-algorithms.com/data_structures/randomized_heap.html).

<span id="heap-nghiêng"></span>

### Đống nghiêng

Đống nghiêng là dạng tự thích nghi của cây lệch trái.
Khi hợp nhất hai đống, nó đổi chỗ hai con ở mọi nút trên đường hợp nhất để cố gắng duy trì cân bằng.
Theo phân tích khấu hao, với đống nghiêng từ trên xuống (top-down skew heap),
...
[Luogu P3377【Mẫu】Cây lệch trái (đống có thể hợp nhất)](https://www.luogu.com.cn/problem/P3377)
...
1.  Trước khi hợp nhất, cần kiểm tra xem hai nút đã ở trong cùng một đống hay chưa.

2.  Độ sâu của cây lệch trái có thể đạt $O(n)$, vì vậy muốn tìm gốc của đống chứa một nút thì phải dùng DSU để duy trì,
...
Trong dạng bài này, thường mỗi nút duy trì một đống, hợp nhất với các con, rồi tùy đề mà pop, sửa đổi và tính đáp án.
...
Trước hết, để tìm gốc của đống chứa một nút, cần dùng DSU, không thể lần ngược cha tuyến tính.
...
Nếu chỉ gốc đống có đánh dấu thì có thể truy vấn nhanh, nhưng làm thế nào để đạt được điều đó?

Có thể dùng cách tương tự hợp nhất theo kích thước: mỗi lần hợp nhất, đẩy đánh dấu của đống nhỏ hơn xuống từng nút bằng
duyệt tuyến tính, rồi dùng đánh dấu của đống lớn hơn làm đánh dấu của đống sau hợp nhất. Vì sau khi hợp nhất sẽ dùng
đánh dấu của đống còn lại, khi đẩy đánh dấu của đống nhỏ hơn xuống cần đẩy lượng bằng đánh dấu của nó trừ đánh dấu của
đống kia. Mỗi lần một nút được hợp nhất, kích thước đống chứa nó ít nhất nhân đôi, nên mỗi nút bị đẩy đánh dấu xuống
...
Có thể dùng cây cân bằng, đống hỗ trợ xóa nút bất kỳ (như cây lệch trái), hoặc `multiset` để duy trì gốc đống của mỗi
đống.

Vì vậy, các thao tác lần lượt như sau:

1.  Đẩy đánh dấu của đống có ít nút hơn bằng duyệt tuyến tính, hợp nhất hai đống, cập nhật kích thước và đánh dấu,
    rồi xóa khỏi `multiset` gốc đống cũ không còn là gốc đống sau hợp nhất.
2.  Xóa nút, cập nhật giá trị, chèn lại, rồi cập nhật multiset. Cần xét riêng trường hợp nút bị xóa có phải là gốc hay
    không.
3.  Gắn đánh dấu lên gốc đống, cập nhật `multiset`.
4.  Gắn đánh dấu toàn cục.
5.  Truy vấn giá trị + đánh dấu của gốc đống + đánh dấu toàn cục.
6.  Truy vấn giá trị của gốc + đánh dấu của gốc đống + đánh dấu toàn cục.
...
[^ref1]: [Đống tự điều chỉnh](https://epubs.siam.org/doi/10.1137/0215004)
