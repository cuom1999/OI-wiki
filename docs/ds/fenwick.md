...
Trước hết xét một ví dụ: cần tính tổng tiền tố $a[1 \ldots 7]$.
...
Thông tin cần thỏa mãn tính kết hợp; nếu không, không thể gộp các đoạn như trên.
...
$c[x]$ luôn quản lý tổng thông tin của một đoạn có biên phải là $x$. Tạm thời chưa cần quan tâm biên trái; trước hết xét cách truy vấn của cây Fenwick.
...
Các phần tử $c$ vừa tìm được là $c_7, c_6, c_4$; đó chính là ba đoạn nhỏ tách ra từ $a[1 \ldots 7]$. Gộp lại sẽ được đáp án $c_7 + c_6 + c_4$.

Ví dụ: tính tổng $a[4 \ldots 7]$.

Vẫn bắt đầu từ $c_7$, nhảy tới $c_6$ rồi tới $c_4$. Lúc này $c_4$ quản lý tổng của $a[1 \ldots 4]$, nhưng phần $a[1 \ldots 3]$ không thuộc truy vấn. Chỉ cần trừ đi tổng $a[1 \ldots 3]$.
...
Cách tính `lowbit` xuất phát từ phép toán bit: `lowbit(x) = x & -x`.

??? note "Nguyên lý của lowbit"
    Đảo tất cả bit trong biểu diễn nhị phân của `x`, rồi cộng thêm 1, sẽ nhận được mã nhị phân của `-x`. Ví dụ, mã nhị phân của $6$ là `110`; đảo toàn bộ bit được `001`, cộng `1` được `010`.
    
    Giả sử mã nhị phân ban đầu của `x` là `(...)10...00`; đảo toàn bộ bit được `[...]01...11`, cộng `1` được `[...]10...00`, tức mã nhị phân của `-x`. Bit `1` đầu tiên trong biểu diễn này của `x` chính là bit `1` thấp nhất của `x`.
...
Tiếp theo xét cách cài đặt các thao tác cụ thể của cây Fenwick, bắt đầu từ truy vấn đoạn.
...
Quá trình truy vấn $a[1 \ldots x]$ có thể viết như sau:
...
Trước khi trình bày sửa đổi một điểm, cần xét một số tính chất cơ bản của cây Fenwick và nguồn gốc dạng cây của nó. Phần này giúp giải thích thao tác sửa đổi một điểm rõ hơn.
...
??? note "Chứng minh"
    ...
    Có $\operatorname{lowbit}(x) = \operatorname{lowbit}(b)$. Lại vì $b - \operatorname{lowbit}(b) \ge 0$,
...
??? note "Chứng minh"
    ...
    Có $\operatorname{lowbit}(y) \ge 2^{k + 1}$, nên $l(y) = (s + 1) \times 2^{k + 1} - \operatorname{lowbit}(y) + 1 \le s \times 2^{k +1} + 1= l(x)$, tức $l(y) \le l(x) \le x < y$.
...
??? note "Chứng minh"
    ...
    Có $\operatorname{lowbit}(y) = \operatorname{lowbit}(b)$. Lại vì $b - \operatorname{lowbit}(b) \ge 0$,
    
    nên $l(y) = y - \operatorname{lowbit}(y) + 1 = x + b - \operatorname{lowbit}(b) + 1 > x$, tức $l(x) \le x < l(y) \le y$.
    
    Vì vậy, $c[x]$ và $c[y]$ không giao nhau.

Sau ba tính chất làm nền, xét dạng cây của cây Fenwick (bỏ qua các cạnh từ $a$ tới $c$).
...
??? note "Chứng minh"
    Đặt $y = x + \operatorname{lowbit}(x)$, $x = s \times 2^{k + 1} + 2^k$, khi đó $y = (s + 1) \times 2^{k +1}$. Có $\operatorname{lowbit}(y) \ge 2^{k + 1} > \operatorname{lowbit}(x)$, chứng minh xong.
...
??? note "Định nghĩa chiều cao"
    ...
    Khái niệm chiều cao được đưa ra để phần giải thích độ phức tạp phía sau thuận tiện hơn.
...
??? note "Chứng minh"
    ...
    Xét một con $v$ của $u$, có $v + \operatorname{lowbit}(v) = u$, tức $v = u - 2^t$ và $\operatorname{lowbit}(v) = 2^t$. Đặt $u = s \times 2^{k + 1} + 2^k$.
...
-   Các đoạn quản lý bởi $c$ ứng với mọi con của $u$ ghép lại đúng bằng $[l(u), u - 1]$.
    ...
    -   Hợp của ba đoạn quản lý trên đúng bằng `a[...0001 ~ ...0111]`, tức $[l(u), u - 1]$.

??? note "Chứng minh"
    Con của $u$ luôn biểu diễn được dưới dạng $u - 2^t(0 \le t < k)$. Giá trị $t$ càng nhỏ thì $u - 2^t$ càng lớn, nên đoạn tương ứng càng nằm về bên phải. Đặt $f(t) = u - 2^t$, khi đó $f(k - 1), f(k - 2), \ldots, f(0)$ lần lượt là các con của $u$ từ trái sang phải.
    
    Có $\operatorname{lowbit}(f(t)) = 2^t$, nên $l(f(t)) = u - 2^t - 2^t + 1 = u - 2^{t + 1} + 1$.
...
Độ phức tạp không gian là $\Theta(n)$.
...
Tương tự, xét việc chuyển truy vấn tổng đoạn thành truy vấn tổng tiền tố bằng sai phân. Với truy vấn tổng của $a[1 \ldots r]$, tức $\sum_{i=1}^{r} a_i$, có:
...
Quan sát công thức này, mỗi $d_j$ được cộng tổng cộng $r - j + 1$ lần. Tiếp tục suy diễn:
...
Cách cộng đoạn được suy ra từ ảnh hưởng của việc cộng $v$ vào đoạn $a[l \ldots r]$ của mảng ban đầu đối với $d$.
...
Theo nguyên lý này, cũng có thể cài đặt "nhân đoạn, hỏi tích đoạn", "xor một số trên đoạn, hỏi xor đoạn", v.v., miễn là thông tin được duy trì và thao tác trên đoạn thuộc cùng một phép toán.
...
Tương tự cây Fenwick một chiều, dùng $c(x, y)$ để biểu diễn tổng thông tin của ma trận $a(x - \operatorname{lowbit}(x) + 1, y - \operatorname{lowbit}(y) + 1) \ldots a(x, y)$. Đây là ma trận có góc dưới phải là $a(x, y)$, cao $\operatorname{lowbit}(x)$ và rộng $\operatorname{lowbit}(y)$.
...
??? note "Chứng minh tính đúng đắn"
    Giả sử $c(p, q)$ quản lý $a(x, y)$, cần tìm phạm vi giá trị của $p$ và $q$.
...
??? note "Chứng minh tính đúng đắn"
    ...
    Nếu xem hàm $t(x)$ như một cây Fenwick, cấu trúc này tương đương với một cây Fenwick lồng trong một cây Fenwick khác; đây chính là nguồn gốc của tên gọi "cây Fenwick lồng cây Fenwick".
        ...
        void add(int x, int y, int v) {
          for (int i = x; i <= n; i += lowbit(i)) {
            for (int j = y; j <= m; j += lowbit(j)) {
              // Cần tạo biến vòng lặp, không thể dùng trực tiếp while (x <= n) như mảng một chiều.
              c[i][j] += v;
            }
          }
        }
...
Tương tự bài toán "cộng đoạn, hỏi tổng đoạn" của cây Fenwick một chiều, xét việc duy trì mảng sai phân.
...
??? note "Vì sao định nghĩa như vậy?"
    ...
    Vì vậy, nếu $a$ là mảng gốc và $d$ là mảng sai phân, có:
...
Ở cách dùng thông thường, cây Fenwick được xây trực tiếp trên dãy ban đầu; $c_6$ biểu diễn thông tin đoạn $a[5 \ldots 6]$.

Tuy nhiên, cũng có thể xây cây Fenwick trên mảng tần suất giá trị của dãy ban đầu; đây chính là cây Fenwick theo giá trị.
...
??? note "Mảng tần suất giá trị là gì?"
    ...
    Kích thước của $b$ liên quan tới miền giá trị của $a$.
    
    Nếu miền giá trị của dãy ban đầu quá lớn, và điều quan trọng không phải giá trị cụ thể mà là quan hệ thứ tự tương đối giữa các giá trị, thường [rời rạc hóa](../misc/discrete.md) mảng ban đầu rồi mới xây mảng tần suất giá trị.
...
Dùng cây Fenwick theo giá trị có thể giải một số bài toán kinh điển.
...
-   Sau khi đổi thứ tự, thực chất đang đếm số cặp thỏa mãn $i \le j$ và $a[i] \ge a[j]$; nhưng khi $i = j$ luôn có $a[i] \ge a[j]$, nên $i \le j$ **không tương đương** với $i < j$, và **không tương đương** với bài toán gốc.
