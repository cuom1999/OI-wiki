...
Đầu tiên, ta chia dãy thành ba khối: $\{1,2,3\},\{4,5,6\},\{7,8,9\}$.
...
(Với các trường hợp không hợp lệ $i>j$, ta giả sử đáp án là $0$.)
...
??? note "Chứng minh chiều cao của cây"
    Theo định nghĩa, gọi $T(n)$ là chiều cao của cây con tại nút "quản lý" $n$ phần tử. Ta có hệ thức truy hồi:
    ...
    Theo định lý chính, ta có $S(m)=O(\log m)$, vì vậy $T(n)=S(\log n)=O(\log\log n)$.
...
Để làm được điều này, ta cần thêm một số phần tử $0$ vào cuối dãy sao cho độ dài dãy trở thành một lũy thừa nguyên của $2$. Dù một số khối có thể lớn gấp đôi so với ban đầu, kích thước đó vẫn là $O(\sqrt{k})$, nên độ phức tạp tiền xử lý phân khối vẫn là $O(n)$.

Bây giờ ta có thể dễ dàng xác định một đoạn truy vấn có nằm hoàn toàn trong một khối hay không. Với đoạn $[l,r]$ (đánh số từ $0$), ta viết hai đầu mút ở dạng nhị phân. Ví dụ, với $k=4, l=39, r=46$, biểu diễn nhị phân là
...
Ta biết độ dài đoạn trên mỗi tầng là như nhau, và kích thước khối cũng như nhau (trong ví dụ trên, $2^k=2^4=16$). Các khối phủ kín toàn bộ dãy, nên khối thứ nhất biểu diễn các phần tử $[0,15]$ (dạng nhị phân là $[000000_2,001111_2]$), khối thứ hai biểu diễn đoạn phần tử $[16,31]$ (dạng nhị phân là $[010000_2,011111_2]$), và cứ tiếp tục như vậy. Ta thấy rằng vị trí của các phần tử trong cùng một khối chỉ khác nhau ở $k$ bit cuối trong biểu diễn nhị phân (trong ví dụ trên $k=4$). Hai đầu mút $l,r$ của ví dụ cũng chỉ khác nhau ở $k$ bit cuối, nên chúng nằm trong cùng một khối.

Vì vậy ta chỉ cần kiểm tra hai đầu mút của đoạn có chỉ khác nhau ở $k$ bit cuối hay không, tức là $l\oplus r\le 2^k-1$. Nhờ đó ta có thể nhanh chóng tìm tầng chứa đoạn cần trả lời:
...
Do đó ta có thể cập nhật cây $index$ như sau:

1.  Cập nhật $\left\langle P_i\right\rangle$ và $\left\langle S_i\right\rangle$ trong thời gian $O(\sqrt{n})$.
2.  Cập nhật $index$. Độ dài của nó là $O(n)$, nhưng ta chỉ cần cập nhật một phần tử trong đó (phần tử này đại diện cho khối bị thay đổi). Bước này tốn $O(\sqrt{n})$ thời gian nếu dùng thuật toán cài đặt đơn giản.
...
**Trang này chủ yếu được dịch từ [Sqrt Tree - Algorithms for Competitive Programming](https://cp-algorithms.com/data_structures/sqrt-tree.html), theo giấy phép CC-BY-SA 4.0.**
