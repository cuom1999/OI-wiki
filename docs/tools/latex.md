...
-   Thao tác cần thực hiện sẽ được đánh dấu bằng mũi tên $\rightarrow$;
-   Ký tự cần nhập sẽ được đặt trong khối mã;
...
Với người dùng Windows, cần tải [TeX Live](https://tug.org/texlive/acquire.html) hoặc [MikTeX](https://miktex.org/download). Nếu tốc độ tải từ máy chủ mặc định không ổn định, có thể chọn một máy chủ bản sao CTAN gần vị trí địa lý gần hơn. Bộ cài TeX Live dạng ISO cần được mount rồi chạy `install-tl-advanced.bat` bằng quyền quản trị.
...
Lúc này tệp nguồn sẽ được chuyển thành tài liệu PDF, quá trình này sẽ mất một ít thời gian. Sau khi biên dịch xong, trình xem PDF của TeXworks sẽ mở và hiển thị tệp đã tạo. Tệp PDF sẽ tự động được lưu trong cùng thư mục với tài liệu TeX.
...
Lệnh `\maketitle` có thể tạo tiêu đề cho tài liệu. Cần chỉ định tiêu đề của tài liệu. Nếu không chỉ định ngày tháng, LaTeX sẽ dùng thời điểm hiện tại; tác giả là tùy chọn.
...
Tài liệu bây giờ trông như sau:
...
-   `\today` là lệnh chèn ngày hiện tại. Cũng có thể nhập một ngày khác, chẳng hạn `\date{November 2013}`.
-   Phần thân của tài liệu **article** sẽ được dàn ngay sau tiêu đề trên cùng một trang. **report** sẽ đặt tiêu đề ở một trang riêng.

### Chương mục

Nếu cần, có thể chia tài liệu thành chương, mục và tiểu mục. Các lệnh chia mục sau dùng được cho tài liệu kiểu **article**:
...
Có thể tạo nhãn cho bất kỳ lệnh chương mục nào để tham chiếu đến chúng ở phần khác của tài liệu. Dùng `\label{labelname}` để tạo nhãn cho chương mục. Sau đó nhập `\ref{labelname}` hoặc `\pageref{labelname}` để tham chiếu đến chương mục tương ứng.
...
$\rightarrow$ Biên dịch và kiểm tra tài liệu PDF. Có thể cần biên dịch liên tiếp hai lần:

![Kết quả tham chiếu nhãn và số trang trong LaTeX](images/latex-for-beginners-2.png)

### Tạo mục lục

Nếu dùng các lệnh chia mục, có thể tạo mục lục. Dùng `\tableofcontents` để tạo mục lục trong tài liệu. Thông thường, mục lục được đặt ngay sau tiêu đề.

Cũng có thể đổi số trang sang chữ số La Mã (i,ii,iii). Việc này bảo đảm phần nội dung chính của tài liệu bắt đầu từ trang 1. Có thể dùng `\pagenumbering{...}` để chuyển đổi giữa số Ả Rập và số La Mã.
...
Khi biên dịch tài liệu, dùng `xelatex` hoặc `lualatex`. Nếu đổi sang phông chữ khác, cần bảo đảm phông chữ đó có đủ các dấu tiếng Việt.
...
$\rightarrow$ Thêm nhiều văn bản hơn vào tài liệu và thử các hiệu ứng phông chữ khác nhau.
...
Trong đó **colorname** là tên màu cần dùng, còn **text** là nội dung văn bản có màu. Lưu ý rằng trong hình ví dụ, chữ màu vàng và trắng có màu nền; điều này cũng có thể thực hiện bằng lệnh `\colorbox` trong gói Color. Cách dùng như sau:
...
Cũng có thể thêm tham số để gọi thêm nhiều màu hơn, thậm chí tự định nghĩa màu cần dùng. Tuy nhiên phần này nằm ngoài phạm vi của sách. Nếu muốn tìm hiểu thêm về văn bản có màu, xem chương [Colors](http://en.wikibooks.org/wiki/LaTeX/Colors) của LaTeX Wikibook.
...
$\rightarrow$ Thử điều chỉnh cỡ chữ cho văn bản.

### Thụt đầu dòng đoạn văn

Theo mặc định, trong LaTeX, dòng đầu của đoạn đầu tiên trong mỗi chương mục không thụt vào, còn các đoạn sau thì thụt dòng đầu. Nếu muốn một đoạn không thụt đầu dòng, thêm lệnh `\noindent` trước đoạn đó. Nếu muốn tất cả các đoạn từ một vị trí trở đi đều không thụt đầu dòng, dùng lệnh `\setlength{\parindent}{0pt}` tại vị trí đó trong tài liệu.
...
Có thể dùng tham số trong ngoặc vuông để thay đổi ký hiệu đầu dòng của danh sách không thứ tự. Ví dụ, `\item[-]` sẽ dùng dấu gạch làm ký hiệu; thậm chí có thể dùng một từ, chẳng hạn `\item[One]`.
...
Dùng `%` để tạo chú thích một dòng. Mọi nội dung trên cùng dòng sau ký tự này sẽ bị bỏ qua cho đến khi bắt đầu dòng tiếp theo.
...
Nếu muốn thêm khoảng cách trong tài liệu, có thể dùng lệnh `\vspace{...}`. Lệnh này thêm khoảng cách theo chiều dọc, với chiều cao có thể chỉ định. Ví dụ `\vspace{12pt}` sẽ tạo một khoảng trống có chiều cao bằng chiều cao của chữ cỡ 12pt.
...
Để dùng các ký tự này, cần thêm dấu gạch chéo ngược phía trước chúng để escape:

```tex
\# \$ \% \^{} \& \_ \{ \} \~{}
```

Lưu ý rằng khi dùng ký tự `^` và `~`, cần đặt ngay sau đó một cặp ngoặc nhọn đóng mở; nếu không, chúng sẽ được hiểu là dấu phía trên chữ cái, giống như `\^ e` sẽ trở thành $\mathrm {\hat{e}}$. Đoạn mã trên tạo ra kết quả sau:

![Kết quả escape các ký tự đặc biệt trong LaTeX](images/latex-for-beginners-11.png)

Lưu ý rằng dấu gạch chéo ngược không thể escape bằng chính dấu gạch chéo ngược, vì như vậy nó sẽ trở thành lệnh xuống dòng; dùng lệnh `\textbackslash` thay thế.

$\rightarrow$ Nhập mã để tạo nội dung sau trong tài liệu:

![Bài thực hành nhập ký tự đặc biệt trong LaTeX](images/latex-for-beginners-12.png)

Có thể hỏi chuyên gia hoặc xem [mã nguồn](https://github.com/OI-wiki/OI-wiki/blob/master/docs/tools/latex.md?plain=1) của trang này để được trợ giúp.

## Bảng

Lệnh bảng (tabular) dùng để dàn trang bảng. Theo mặc định, bảng trong LaTeX không có đường phân cách ngang và dọc; nếu cần, phải tự đặt. LaTeX sẽ tự động đặt chiều rộng bảng theo nội dung. Đoạn mã sau có thể tạo một bảng:
...
Ví dụ, `{lll}` sẽ tạo một bảng ba cột, tất cả đều căn trái và không có đường kẻ dọc; `{|l|l|r|}` sẽ tạo một bảng ba cột, hai cột đầu căn trái, cột cuối căn phải, và có đường kẻ dọc giữa các cột liền kề.
...
Chương này giới thiệu cách chèn hình ảnh vào tài liệu LaTeX. Trong phần này cần nạp gói **graphicx**. Hình ảnh nên là tệp PDF, PNG, JPEG hoặc GIF. Đoạn mã sau sẽ chèn một hình tên myimage:
...
`[h]` là tham số vị trí; **h** nghĩa là đặt hình ảnh xấp xỉ tại đây, nếu có đủ chỗ. Các tùy chọn khác gồm: **t** nghĩa là đặt ở đầu trang; **b** nghĩa là đặt ở cuối trang; **p** nghĩa là đặt hình ảnh ở một trang riêng. Cũng có thể thêm tham số **!** để ép đặt ở vị trí do tham số chỉ định, mặc dù kết quả dàn trang có thể không đẹp.
...
Lệnh `\includegraphics{...}` có thể tự động đặt hình vào tài liệu; tệp ảnh nên nằm cùng thư mục với tệp TeX.

`[width=1\textwidth]` là tham số tùy chọn chỉ định chiều rộng của hình, trong ví dụ này bằng chiều rộng phần văn bản. Chiều rộng cũng có thể được đặt theo đơn vị xentimét. Cũng có thể dùng `[scale=0.5]` để thu nhỏ hình theo tỷ lệ; ví dụ này tương đương thu nhỏ một nửa.

`\caption{...}` định nghĩa tiêu đề của hình ảnh. Nếu dùng lệnh này, LaTeX sẽ thêm số thứ tự bắt đầu bằng "Figure" ("Hình") cho hình ảnh. Có thể dùng `\listoffigures` để tạo danh mục hình ảnh.

`\label{...}` tạo một nhãn để có thể tham chiếu.
...
Thay **ImageFilename** bằng tên tệp, không bao gồm phần mở rộng. Nếu tên tệp có khoảng trắng, dùng dấu nháy kép bao quanh, chẳng hạn `"screen 20"`.
...
Có thể dùng một cặp `$` để bật chế độ toán học; cách này dùng để viết công thức toán trong dòng. Ví dụ `$1+2=3$` tạo ra kết quả $1+2=3$.
...
Số 6 là số thứ tự của chương, chỉ xuất hiện khi tài liệu có đặt chương, chẳng hạn tài liệu kiểu **report**.
...
Tài liệu này chỉ là phần nhập môn về cách dùng ký hiệu toán học; chương ký hiệu toán học của LaTeX Wikibook là một hướng dẫn khác tốt hơn và đầy đủ hơn. Nếu muốn tìm hiểu thêm về ký hiệu toán học, xem tài liệu đó. Nếu muốn tìm một ký hiệu cụ thể, có thể dùng [Detexify](http://detexify.kirelabs.org), công cụ có thể nhận diện ký tự viết tay.
...
Dùng lệnh `\sqrt{...}` để chèn căn thức. Nội dung trong dấu ba chấm được thay bằng biểu thức lấy căn. Nếu cần thêm bậc của căn, đặt bậc đó trong ngoặc vuông.
...
Có thể dùng dấu gạch chéo ngược kèm tên chữ cái Hy Lạp để biểu thị một chữ cái Hy Lạp. Việc viết hoa hay viết thường chữ cái đầu của tên quyết định dạng của chữ cái Hy Lạp. Ví dụ:
...
Nếu cần trợ giúp, có thể xem [mã nguồn](https://github.com/OI-wiki/OI-wiki/blob/master/docs/tools/latex.md?plain=1) của trang này.
...
Tệp BibTeX chứa tất cả tài liệu cần trích dẫn trong tài liệu đang viết. Phần mở rộng của nó là `.bib`. Tên của nó nên được đặt theo tên tài liệu TeX. Tệp `.bib` là tệp văn bản. Cần nhập tài liệu tham khảo theo định dạng sau:
...
Có thể dùng lệnh LaTeX để tạo hiệu ứng văn bản đặc biệt. Ví dụ chữ nghiêng có thể dùng `\emph{Rattus norvegicus}`.

Với các chữ cái cần viết hoa, dùng ngoặc nhọn bao lại. BibTeX sẽ tự động đổi tất cả chữ hoa trong tiêu đề, trừ chữ cái đầu tiên, thành chữ thường. Ví dụ `Dispersal in the contemporary United States` tạo ra kết quả $\text{Dispersal in the contemporary united states}$, còn `Dispersal in the contemporary {U}nited {S}tates` tạo ra kết quả $\text{Dispersal in the contemporary United States}$.

Có thể viết tệp BibTeX thủ công, hoặc dùng phần mềm để tạo.
...
Dùng `\cite{citationkey}` để chèn một chú thích trích dẫn tại nơi cần trích dẫn tài liệu. Nếu không muốn chèn chú thích trích dẫn trong phần thân văn bản nhưng vẫn muốn hiển thị mục trích dẫn này trong danh sách tài liệu, dùng lệnh `\nocite{citationkey}`.
...
-   LaTeX Wikibook <http://en.wikibooks.org/wiki/LaTeX/> Đầy đủ và được viết mạch lạc, dù vẫn đang được hoàn thiện. Cũng có bản PDF để tải xuống.

-   Comparison of TeX Editors on Wikipedia <http://en.wikipedia.org/wiki/Comparison_of_TeX_editors> Thông tin giúp chọn trình soạn thảo LaTeX để cài trên máy cá nhân.

-   TeX Live <http://www.tug.org/texlive/> "Một cách đơn giản để bắt đầu dùng hệ thống soạn thảo tài liệu TeX". Có cho Unix và Windows (có liên kết tới MacTeX cho người dùng MacOSX). Bao gồm trình soạn thảo TeXworks.
