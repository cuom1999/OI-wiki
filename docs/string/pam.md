...
Vì độ dài xâu đối xứng có thể là lẻ hoặc chẵn, ta có thể làm giống Manacher: thêm một ký tự không thuộc bảng chữ cái (chẳng hạn `#`) làm dấu phân cách để biến mọi xâu đối xứng thành độ dài lẻ. Tuy nhiên cách này khá rườm rà. Có cách nào tốt hơn không?
...
Giống như các automaton khác, con trỏ fail của một nút trỏ tới nút tương ứng với hậu tố đối xứng dài nhất của xâu đối xứng mà nút đó biểu diễn. Tuy nhiên cạnh chuyển không biểu diễn việc thêm một ký tự vào sau xâu ở nút ban đầu, mà biểu diễn việc thêm cùng một ký tự vào cả đầu và cuối xâu đó (điều này tự nhiên vì ta cần bảo đảm xâu được lưu là đối xứng).
...
Con trỏ fail của gốc chẵn trỏ tới gốc lẻ. Ta không cần quan tâm con trỏ fail của gốc lẻ, vì gốc lẻ không thể thất bại khi khớp (trạng thái tiếp theo chuyển ra từ gốc lẻ có độ dài $1$, tức là một ký tự đơn, chắc chắn là xâu con đối xứng).

Tương tự automaton hậu tố, ta xây cây hồi văn theo kiểu tăng dần.

Xét sau khi đã xây xong cây hồi văn cho $p-1$ ký tự đầu, ta thêm vào automaton ký tự ở vị trí $p$ trong xâu gốc.

Ta bắt đầu từ nút tương ứng với xâu con đối xứng dài nhất kết thúc tại ký tự trước đó, rồi liên tục đi theo con trỏ fail cho tới khi tìm được một nút thỏa $s_{p}=s_{p-len-1}$, tức là ký tự đứng ngay trước xâu con đối xứng mà nút đó biểu diễn bằng với ký tự cần thêm.

Dưới đây là hình trong bài báo:

![](./images/pam2.png)

Ta tìm được nút tương ứng với `A` bằng cách nhảy theo con trỏ fail, rồi thêm `X` vào hai bên để thu được xâu đối xứng hiện tại (tức `XAX`). Rõ ràng nút này chính là nút trên cây tương ứng với xâu con đối xứng dài nhất kết thúc tại $p$. Lúc này lợi thế của nút độ dài $-1$ cũng thể hiện rõ: nếu không có `X` nào khác khớp điều kiện, phép so sánh sẽ trở thành $s_p=s_p$ tại cùng một vị trí, nhờ đó tự nhiên thu được nút biểu diễn ký tự `X`. Sau đó cần kiểm tra: nếu chưa có nút này thì phải tạo mới.

Tiếp theo ta cần tìm con trỏ fail cho nút mới tạo. Cách làm tương tự quá trình trên: bắt đầu từ `A`, liên tục nhảy theo con trỏ fail để tìm hậu tố đối xứng dài nhất `XBX` của `XAX`, rồi đặt nút tương ứng làm đích của con trỏ fail.

Rõ ràng nút này không cần tạo mới. $len_B$ ký tự đầu và $len_B$ ký tự cuối của `A` đều giống nhau và đều là `B`; theo quan hệ đối xứng, hai đầu của $len_B$ ký tự đầu đều là `X`, còn phía sau đã được xác định là `X`, nên nút `XBX` chắc chắn đã được chứa trong cây.
...
-   Khi $|s| >1$, đặt $t=sc$, trong đó $t$ là xâu thu được sau khi thêm một ký tự $c$ vào cuối $s$. Giả sử kết luận đúng với xâu $s$. Xét các xâu con đối xứng kết thúc tại ký tự cuối $c$, và giả sử các đầu trái của chúng theo thứ tự tăng dần là $l_1,l_2,\dots,l_k$. Vì $t[l_1..|t|]$ là xâu đối xứng, nên với mọi vị trí $l_1 \le p \le |t|$, ta có $t[p..|t|]=t[l_1..l_1+|t|-p]$. Do đó, với $1 < i \le k$, $t[l_i..|t|]$ đã xuất hiện trong $t[1..|t|-1]$. Vì vậy, mỗi lần thêm một ký tự, số xâu con đối xứng phân biệt bản chất tăng thêm nhiều nhất $1$.
...
Với việc xây cây hồi văn của $s$, đặt $n=|s|$. Rõ ràng mọi thao tác ngoài việc nhảy theo con trỏ fail đều là $O(n)$.
...
Định nghĩa giá trị tồn tại của một xâu con của $s$ là số lần xâu con đó xuất hiện trong $s$ nhân với độ dài của xâu con đó. Với xâu $s$ cho trước, hãy tìm giá trị tồn tại lớn nhất trong mọi xâu con đối xứng.
...
Xét quy hoạch động, gọi $dp[i]$ là số phần ít nhất trong một phân hoạch của tiền tố độ dài $i$ của $s$. Khi chuyển trạng thái, ta chỉ cần duyệt mọi xâu đối xứng kết thúc tại ký tự thứ $i$:
...
???+ note "Chứng minh"
    Với $1 \le i \le |t|$, vì $s$ và $t$ đều là xâu đối xứng, ta có $s[i]=s[|s|-i+1]=s[|s|-|t|+i]$, nên $t$ là border của $s$.
    
    Với $1 \le i \le |t|$, vì $t$ là border của $s$, ta có $s[i]=s[|s|-|t|+i]$; vì $s$ là xâu đối xứng, ta có $s[i]=s[|s|-i+1]$. Do đó $s[|s|-i+1]=s[|s|-|t|+i]$, nên $t$ là xâu đối xứng.
...
Theo kết luận đã chứng minh ở trên, nếu dùng con trỏ $slink$ để nhảy lên trên, thì mỗi khi thêm một ký tự vào cuối chỉ cần nhảy lên $O(\log |s|)$ lần. Vì vậy, ta có thể ghi tổng các giá trị $dp$ (trong bài toán gốc là $\min$) của mọi xâu đối xứng được biểu diễn bởi một cấp số cộng vào nút tương ứng với xâu đối xứng dài nhất trong cấp số cộng đó.
