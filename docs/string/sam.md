...
???+ note "Giải thích chi tiết thuật toán"
    -   Nếu một chuyển trạng thái $(p,q)$ thỏa $\operatorname{len}(p)+1=\operatorname{len}(q)$, gọi chuyển trạng thái này là **liên tục**. Ngược lại, khi $\operatorname{len}(p)+1<\operatorname{len}(q)$, chuyển trạng thái đó được gọi là **không liên tục**.
        ...
        Từ mô tả thuật toán có thể thấy các chuyển trạng thái liên tục và không liên tục được xử lý khác nhau. Chuyển trạng thái liên tục là cố định và sẽ không thay đổi nữa. Ngược lại, khi chèn một ký tự mới vào xâu, chuyển trạng thái không liên tục có thể thay đổi (đầu mút của cạnh chuyển có thể thay đổi).
