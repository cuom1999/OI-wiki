...
Sàng số lũy thừa mạnh (powerful number sieve) tương tự sàng Dujiao, hoặc có thể xem là một mở rộng của sàng Dujiao, dùng để tính tổng tiền tố của một số hàm nhân.
...
**Định nghĩa**: Với số nguyên dương $n$, giả sử phân tích thừa số nguyên tố của $n$ là $n = \prod_{i=1}^{m} p_{i}^{e_{i}}$. $n$ là số lũy thừa mạnh khi và chỉ khi $\forall 1 \le i \le m, e_{i} > 1$.

**Tính chất 1**: Mọi số lũy thừa mạnh đều có thể biểu diễn dưới dạng $a^{2}b^{3}$.
...
**Tính chất 2**: Số lượng số lũy thừa mạnh không vượt quá $n$ nhiều nhất là $O(\sqrt{n})$.

**Chứng minh**: Xét việc liệt kê $a$, rồi xét số lượng $b$ thỏa mãn điều kiện. Số lượng số lũy thừa mạnh xấp xỉ bằng

$$
\int_{1}^{\sqrt{n}} \sqrt[3]{\frac{n}{x^2}} \mathrm{d}x = O(\sqrt{n})
$$

Vậy làm thế nào để tìm mọi số lũy thừa mạnh không vượt quá $n$? Dùng sàng tuyến tính tìm mọi số nguyên tố không vượt quá $\sqrt{n}$, rồi tìm kiếm theo chiều sâu trên số mũ của từng số nguyên tố. Vì số lượng số lũy thừa mạnh không vượt quá $n$ nhiều nhất là $O(\sqrt{n})$, nên số lần tìm kiếm cũng nhiều nhất là $O(\sqrt{n})$.

<span id="sàng-pn"></span>
## Sàng số lũy thừa mạnh
...
Với số nguyên tố $p$, $f(p) = g(1)h(p) + g(p)h(1) = h(p) + g(p) \implies h(p) = 0$. Từ $h(p)=0$ và việc $h$ là hàm nhân, suy ra với mọi số $n$ không phải số lũy thừa mạnh thì $h(n) = 0$, tức là $h$ chỉ có giá trị khác không tại các số lũy thừa mạnh.
...
F(n) &= \sum_{i = 1}^{n} f(i)\\
     ...
     &= \sum_{\substack{d=1 \\ d \text{ là số lũy thừa mạnh}}}^{n}h(d) G\left(\left\lfloor \frac{n}{d}\right\rfloor\right)
\end{aligned}
$$

Dùng $O(\sqrt{n})$ để tìm mọi số lũy thừa mạnh và tính mọi giá trị khác không của $h$. Để tính các giá trị khác không của $h$, chỉ cần tính mọi giá trị tại $h(p^c)$, rồi dựa vào việc $h$ là hàm nhân để suy ra mọi giá trị khác không của $h$. Sau đó, với mỗi $d$ làm cho $h(d)\ne 0$, tính $h(d)G\left(\left\lfloor \dfrac{n}{d} \right\rfloor\right)$ và cộng dồn là thu được $F(n)$.
...
3.  Tính $h(p^c)$.
4.  Tìm kiếm các số lũy thừa mạnh, đồng thời cộng dồn đáp án.
5.  Thu được kết quả.
...
Với phần tìm kiếm, vì số lượng số lũy thừa mạnh không vượt quá $n$ nhiều nhất là $O(\sqrt{n})$, nên tìm kiếm nhiều nhất $O(\sqrt{n})$ lần. Với mỗi số lũy thừa mạnh, độ phức tạp phụ thuộc vào cách tính $G$. Ví dụ, nếu độ phức tạp tính $G\left(\left\lfloor \dfrac{n}{d}\right\rfloor\right)$ là $O(1)$, thì độ phức tạp của phần thứ hai là $O(\sqrt{n})$.
