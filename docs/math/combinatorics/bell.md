...
Mỗi số Bell là tổng của các [số Stirling loại hai](./stirling.md#số-stirling-loại-hai-stirling-number) tương ứng.
...
Sau khi tiền xử lý $n$ hạng đầu của $\mathrm{e}^x - 1$, chỉ cần thực hiện một lần [exp đa thức](../poly/elementary-func.md#hàm-logarit-và-hàm-mũ-của-đa-thức) là có thể thu được $n$ số Bell đầu tiên. Nút thắt thời gian nằm ở exp đa thức, có thể đạt độ phức tạp thời gian $O(n \log n)$.
