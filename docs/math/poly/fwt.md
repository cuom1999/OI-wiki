...
Vì vậy, ý tưởng cốt lõi của FWT và FFT về bản chất là giống nhau: đều là biến đổi trên mảng. Ta ký hiệu kết quả sau khi thực hiện biến đổi Walsh nhanh trên mảng $A$ là $FWT[A]$.

Ý tưởng cốt lõi của FWT như sau:

Ta cần một dãy mới $C$, thu được từ hai dãy $A$ và $B$ thông qua một quy tắc phép toán nào đó, tức $C = A \cdot B$.
...
Bây giờ muốn có $FWT[C] = FWT[A] \cdot FWT[B]$, ta cần xây dựng quy tắc cho FWT này.
...
Hiển nhiên không thể duyệt tất cả, vì độ phức tạp sẽ là $O(n^2)$. Khi không thể duyệt toàn bộ, ta xét chia để trị.
...
Như vậy, bằng chia đôi ta có thể hoàn thành việc ghép qua $O(\log{n})$ tầng; mỗi lần ghép cần thực hiện một lượt tính toán. Vì thế ta thu được $FWT[A]$ trong $O(n\log{n})$.

Phần còn lại là biến đổi ngược. Biến đổi ngược thực ra rất đơn giản: đã biết các tập con của $A_0$ chỉ nằm trong chính nó ($A_0 = FWT[A_0]$), còn tập con của $A_1$ là $FWT[A_0] + FWT[A_1]$, nên dễ dàng suy ra công thức truy hồi cho biến đổi ngược:

$$
UFWT[A'] = merge(UFWT[A_0'], UFWT[A_1'] - UFWT[A_0'])
$$

Dưới đây là cài đặt. Dễ thấy biến đổi xuôi và ngược có thể gộp vào một hàm; khi biến đổi xuôi $\text{type}=1$, khi biến đổi ngược $\text{type}=-1$.
...
Nếu đặt $x\circ y$ là tính chẵn lẻ của số bit $1$ trong $x\cap y$, tức $x\circ y=\text{popcnt}(x\cap y)\bmod 2$, thì dễ thấy $(x\circ y)\oplus (x\circ z)=x\circ(y\oplus z)$.

Phép tính $FWT[A]$ cũng có thể suy ra rất tự nhiên.

Đặt $FWT[A]_i=\sum_{i\circ j=0}A_j-\sum_{i\circ j=1}A_j$. Ta chứng minh tính đúng đắn của $FWT[C] = FWT[A] \cdot FWT[B]$:
...
Hãy xem cách tính nhanh giá trị của $A,B$; ta vẫn dùng chia để trị:
...
Nên bằng một chứng minh đơn giản, ta có thể thu được: $c(i,j)c(i,k)=c(i,j\odot k)$. Trong đó $\odot$ là một phép toán bit bất kỳ.
...
Làm trực tiếp như vậy là kém hiệu quả. Ta tách nó thành:

$$
FWT[A]_i = \sum_{j=0}^{n/2-1} c(i,j) A_j+\sum_{j=n/2}^{n-1} c(i,j) A_j
$$

Xét sự khác nhau của $i,j$ trong biểu thức trước và biểu thức sau, ta thấy chỉ có bit cao nhất là khác nhau.

Do đó ta loại bỏ bit cao nhất của $i,j$ để được $i',j'$, và ký hiệu $i_0$ là bit cao nhất của $i$. Khi đó:
...
bốn số này là có thể hoàn thành biến đổi. Ta gọi ma trận này là ma trận bit.

Nếu muốn thực hiện biến đổi ngược, ta cần ma trận nghịch đảo của ma trận bit trên.
...
Như vậy thỏa mãn $c(i,j)c(i,k)=c(i,j\cup k)$. Dễ thấy điều này hoàn toàn trùng với công thức $FWT[A]=\text{merge}(FWT[A_0], FWT[A_0]+FWT[A_1])$ đã suy ra ở trên. Tương tự, ma trận dưới đây cũng thỏa mãn điều kiện này, nhưng thông thường ta dùng ma trận phía trên:
...
Nếu muốn thực hiện biến đổi ngược, ta cần lấy nghịch đảo của ma trận. Lấy ma trận **trên cùng** làm ví dụ, thu được:
...
Nhưng nếu mỗi hàng của ma trận đều giống nhau thì ma trận cũng không có nghịch đảo, nên ta có thể xây dựng $c(i,j)=\omega_{K}^{(i-1)j}$.
...
Nếu modulo của đề bài có căn đơn vị, ta có thể cài đặt đơn giản.

Tuy nhiên **căn đơn vị có thể không tồn tại theo modulo**, nên ta xét mở rộng trường: tự định nghĩa một $x$ thỏa $x^K=1$, rồi thay trực tiếp $x$ vào để tính. Khi đó mỗi số là một đa thức theo $x$ bậc $k-1$. Ta chỉ cần tính dưới modulo $x^K-1$. Khi ấy ma trận có thể biểu diễn như sau:
...
Nhưng cách làm này có thể sinh ước không, tức **một số có nhiều cách biểu diễn**; ta không thể xác định giá trị thật của một số.

Vì thế ta không tính theo $\bmod {x^K-1}$ nữa, mà tính theo $\bmod$ đa thức cyclotomic $\Phi_{K}(x)$. Đa thức này đảm bảo cấp của $x$ là $k$ và bất khả quy trên $\mathbb{Q}$. Do đó, ta định nghĩa phép tính bên trên được thực hiện dưới $\bmod {\Phi_{K}(x)}$.
...
???+ note "[CF 1103E - Radix sum](https://www.luogu.com.cn/problem/CF1103E)"
    Cho một dãy $a_1,a_2,...,a_n$ độ dài $n$. Với mỗi $p \in [0,n-1]$, hãy tính số dãy số nguyên $i_1,i_2,...,i_n$ thỏa mãn các điều kiện sau, lấy modulo $2^{58}$:
    ...
    ??? note "Lời giải"
        Có thể nghĩ đến DP: thiết kế trạng thái $f_{i,s}$ biểu diễn đã xét đến số thứ $i$, và trạng thái phép cộng hiện tại là $s$. Vì biến đổi FWT là tuyến tính, ta có thể biến đổi trước sang biểu diễn giá trị điểm FWT, sau đó đưa thành lũy thừa bậc $n$ của chính nó, cuối cùng biến đổi ngược lại.
        
        Phần trên là trực tiếp, nhưng đề bài cho modulo $2^{58}$. Ta thấy không có căn đơn vị, nên xét mở rộng trường.
        
        Đa thức cyclotomic ở đây là $\Phi_{10}(x)=x^4-x^3+x^2-x+1$.
        
        Tuy nhiên khi UFWT, cần chia cho cơ số $10$, mà $10$ không có nghịch đảo modulo $2^{58}$. Thực ra $5$ có nghịch đảo modulo $2^{58}$: $57646075230342349$; ta chỉ cần chia thêm một thừa số $2$. Giả sử đáp án sau khi đã chia cho $5$ là $x$, đáp án thật là $y$, tức $2^5y\equiv x\pmod{2^{64}}$. Hiển nhiên $y\equiv \frac{x}{2^5}\pmod{2^{64-5}}$, tức $y\equiv \frac{x}{2^5}\pmod{2^{59}}$. Vì vậy chỉ cần chia đáp án cuối cùng cho $2^5$. Mặc dù không rõ vì sao tác giả đề bài lại yêu cầu modulo $2^{58}$, sau đó chỉ cần lấy modulo lại là đủ.
