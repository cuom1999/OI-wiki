// Dùng để giảm kích thước mảng nhằm tiết kiệm bộ nhớ, sẽ kẹt ở 80 điểm
#include <cmath>
#include <iostream>
#include <map>
using namespace std;
...
long long ksm(long long a, long long m) {  // Dùng để tính nghịch đảo
  long long res = 1;
  while (m) {
    if (m & 1) res = res * a % P;
    a = a * a % P, m >>= 1;
...
void prime_work(int k) {  // Sàng tuyến tính phi và s
...
long long s3(long long k) {  // Tổng lập phương
  return k %= P, (k * (k + 1) / 2) % P * ((k * (k + 1) / 2) % P) % P;
}

long long s2(long long k) {  // Tổng bình phương
  return k %= P, k * (k + 1) % P * (k * 2 + 1) % P * inv6 % P;
}

long long calc(long long k) {  // Tính S(k)
  if (k <= pn) return s[k];
  if (s_map[k]) return s_map[k];  // Các giá trị lớn hơn pn được lưu rời rạc bằng map
