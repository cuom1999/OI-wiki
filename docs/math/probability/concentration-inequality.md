...
Khi số tầng tăng dần, các cận trên và cận dưới xuất hiện luân phiên cũng ngày càng chặt hơn. Chuỗi kết luận này có hình thức tương tự nguyên lý bao hàm - loại trừ, và cách chứng minh cũng tương tự bao hàm - loại trừ; phần chứng minh được lược bỏ.

<span id="bất-đẳng-thức-markov"></span>
## Bất đẳng thức Markov

Giả sử $X$ là một biến ngẫu nhiên nhận giá trị không âm, khi đó với mọi số thực dương $a$ có
...
<span id="bất-đẳng-thức-chebyshev"></span>
## Bất đẳng thức Chebyshev

Giả sử $X$ là một biến ngẫu nhiên, khi đó với mọi $a > 0$ đều có

$$
P \{ |X - EX| \geq a \} \leq \frac{DX}{a^2}
$$

Đặc biệt, khi lấy $a$ bằng $k\sigma$, có
...
<span id="bất-đẳng-thức-chernoff"></span>
...
<span id="bất-đẳng-thức-chernoff-cho-tổng-các-phép-thử-bernoulli"></span>
### Bất đẳng thức Chernoff cho tổng các phép thử Bernoulli

Các biến ngẫu nhiên xuất hiện trong thi đấu thuật toán thường không "tổng quát" đến vậy; nhiều biến có thể mô tả bằng các phép thử Bernoulli trong xác suất.

Phép thử Bernoulli là một phép thử ngẫu nhiên chỉ có hai kết quả có thể xảy ra.

Kết quả của một phép thử Bernoulli có thể được mô tả bằng một biến ngẫu nhiên $X$ nhận giá trị $0$ hoặc $1$, với phân phối xác suất
...
Đối với phép thử Bernoulli, có kết luận sau:

Với $n$ phép thử Bernoulli độc lập $X_1, X_2, \cdots, X_n$, đặt $X = \sum_{i=1}^{n} X_i$ và $\mu = EX$, khi đó với mọi $0 < \epsilon < 1$ có

$$
P\left\{ |X - \mu| \geq \epsilon \mu \right\} \leq 2 \exp\left( - \frac{1}{3} \mu \epsilon^2 \right)
$$

<span id="bất-đẳng-thức-hoeffding"></span>
...
Cả bất đẳng thức Chernoff và bất đẳng thức Hoeffding đều giới hạn mức độ một biến ngẫu nhiên lệch khỏi kỳ vọng của nó. Chứng minh của hai bất đẳng thức này khá dài; có thể tham khảo các chương liên quan trong sách Probability and Computing.
...
<span id="ví-dụ-ứng-dụng"></span>
## Ví dụ ứng dụng

<span id="ví-dụ-ước-lượng-số-pi-bằng-cách-gieo-điểm-ngẫu-nhiên"></span>
...
Câu hỏi: để thuật toán trên trả về kết quả có sai số tương đối không vượt quá $\epsilon$ với xác suất ít nhất $(1 - \delta)$, cần chọn $n$ như thế nào?

??? note "Lời giải"
    Ký hiệu $X_i$ biểu diễn biến cố "điểm thứ $i$ được sinh ngẫu nhiên nằm trong đĩa tròn đơn vị", khi đó tổng số điểm trong đĩa tròn là $X = \sum_{i=1}^{n} X_i$. Cần tìm một $n$ thích hợp sao cho
    ...
    Theo bất đẳng thức Chernoff, chỉ cần đặt
    ...
    Tức là khi $n = \Omega(\epsilon^{-2} \ln \frac{1}{\delta})$, có thể đạt được độ chính xác cần thiết.

<span id="ví-dụ-bài-toán-rút-thưởng"></span>
### Ví dụ: bài toán rút thưởng

Trong một hộp có $n$ quả bóng, trong đó đúng $k$ quả bóng tương ứng với giải thưởng lớn. Thực hiện một số lần rút ngẫu nhiên độc lập, đều xác suất; sau mỗi lần rút sẽ đặt bóng trở lại hộp. Hỏi cần rút bao nhiêu lần để với xác suất ít nhất $(1 - \epsilon)$, **mỗi** quả bóng trúng thưởng đều được rút ra ít nhất một lần?
...
??? note "Lời giải"
    ...
    Bây giờ có $k>1$ quả bóng trúng thưởng. Theo cận hợp, chỉ cần xác suất mỗi quả bóng trúng thưởng bị bỏ sót đều không vượt quá $\dfrac \epsilon k$. Do đó đáp án là $n \log \dfrac{k}{\epsilon}$.

<span id="ví-dụ-chọn-ngẫu-nhiên-một-nửa-số-phần-tử"></span>
### Ví dụ: chọn ngẫu nhiên một nửa số phần tử

Yêu cầu đưa ra một thuật toán chọn ngẫu nhiên đều xác suất một tập con có kích thước $\dfrac{n}{2}$ từ $n$ phần tử, trong đó $n$ là số chẵn. Nguồn ngẫu nhiên duy nhất được dùng là một đồng xu công bằng; đồng thời cố gắng giảm số lần tung đồng xu (không yêu cầu tối thiểu).

??? note "Lời giải"
    Trước hết có thể nghĩ đến thuật toán sau:
    
    -   Bằng cách tung đồng xu $n$ lần, có thể chọn ngẫu nhiên đều xác suất một tập con trong tất cả các tập con.
    ...
    -   Có thể thực hiện chọn ngẫu nhiên 1 trong $n$ phần tử bằng cách tung đồng xu $2\lceil\log_2 n\rceil$ lần theo kỳ vọng.
    ...
    -   Sau đó chọn một phần tử từ tất cả các phần tử, rồi chọn thêm một phần tử từ các phần tử còn lại, cứ như vậy cho đến khi chọn được $\dfrac n2$ phần tử.
    ...
    ??? note "Tính độ phức tạp kỳ vọng"
        Tiếp tục phân tích từ một góc nhìn khác, thử tính số lần tung đồng xu kỳ vọng của thuật toán này.
...
$n$ quả bóng được ném độc lập ngẫu nhiên vào $n$ hộp. Chứng minh rằng số bóng trong hộp chứa nhiều bóng nhất không nhỏ hơn $\Omega \left( \dfrac{\log n}{\log \log n} \right)$ với xác suất $1 - \dfrac{1}{n}$.
