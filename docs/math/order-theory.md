...
Khi nghiên cứu quan hệ hai ngôi, thường quan tâm liệu chúng có một số tính chất đặc biệt hay không. Với quan hệ hai ngôi $R$ trên tập $S$, định nghĩa các tính chất đặc biệt sau:
...
8.  Tính có cơ sở tốt (well-founded): mọi tập con không rỗng $T\subseteq S$ đều có phần tử cực tiểu, tức $(\forall T\subseteq S, T\neq\varnothing)(\exists m\in T)(\forall a\in T)~~\lnot(aRm)$,
9.  Tính bắc cầu của không so sánh được (transitive of incomparability): $(\forall~a,b,c \in S)~~(\lnot(aRb \lor bRa) \land \lnot(bRc \lor cRb)) \implies \lnot(aRc \lor cRa)$ (nếu $\lnot(aRb \lor bRa)$, nói rằng $a$ và $b$ không so sánh được).

Đồng thời, định nghĩa một số quan hệ hai ngôi đặc biệt:
...
Với hai quan hệ hai ngôi $R$ và $S$ trên tập $X$ và tập $Y$, có thể định nghĩa các phép toán sau:
...
Với quan hệ hai ngôi $R$ trên tập $X$ và tập $Y$, cùng quan hệ hai ngôi $S$ trên tập $Y$ và tập $Z$, có thể định nghĩa hợp thành $S\circ R$ của chúng sao cho $G(S\circ R):=\{(x,z):(\exists~y\in Y)~~xRy\land ySz\}$.
...
$\mathbf{N}$, $\mathbf{Z}$, $\mathbf{Q}$ và $\mathbf{R}$ đều tạo thành tập có thứ tự toàn phần theo $\leq$.
...
Với tập có thứ tự bộ phận hữu hạn, có thể dùng biểu đồ Hasse để biểu diễn trực quan quan hệ thứ tự bộ phận trên đó.
...
Vì thứ tự bộ phận có tính phản đối xứng, biểu đồ Hasse là [đồ thị có hướng không chu trình](../graph/dag.md). Do đó, có thể dựa vào [sắp xếp topo](../graph/topo.md) để xây dựng một thứ tự toàn phần cho bất kỳ tập có thứ tự bộ phận hữu hạn nào.
...
???+ note "Định nghĩa"
    ...
    Với tập có thứ tự bộ phận $S$ và thứ tự bộ phận $\preceq$ trên đó, kích thước của phản xích lớn nhất trong $S$ được gọi là **độ rộng** (partial order width) của tập có thứ tự bộ phận $S$.
...
Trong tập tiền thứ tự, có thể định nghĩa các khái niệm như phần tử cực đại (cực tiểu), cận trên (cận dưới), cận trên nhỏ nhất (cận dưới lớn nhất). Các khái niệm này có thể được mở rộng sang các quan hệ thứ tự khác.

???+ note "Định nghĩa"
    Với tập tiền thứ tự $S$ và tiền thứ tự $\preceq$ trên đó, lấy phần tử $m$ trong $S$:

    Định nghĩa quan hệ nghiêm ngặt $x\prec y \iff x\preceq y\land\lnot(y\preceq x)$.

    1.  Nếu không tồn tại $a\in S$ sao cho $m\prec a$, thì $m$ được gọi là **phần tử cực đại** (maximal element),
    2.  Nếu với $T \subseteq S$ có $(\forall~t\in T)~~t\preceq m$, thì $m$ được gọi là **cận trên** (upper bound) của $T$,
    3.  Nếu với $T \subseteq S$, $m$ là cận trên của $T$ và với mọi cận trên $n$ của $T$ đều có $m \preceq n$, thì $m$ được gọi là **cận trên nhỏ nhất** (supremum) của $T$.

    Tương tự, có thể định nghĩa **phần tử cực tiểu** (minimal element), **cận dưới** (lower bound) và **cận dưới lớn nhất** (infimum).

Ví dụ, $1$ là phần tử cực tiểu và cận dưới của $\mathbf{N}_+$.

Có thể chứng minh:

-   Trong tập tiền thứ tự, phần tử cực đại (cực tiểu), cận trên (cận dưới), cận trên nhỏ nhất (cận dưới lớn nhất) không nhất thiết tồn tại; ngay cả khi tồn tại cũng không nhất thiết duy nhất.

-   Nếu tập con $T$ của tập có thứ tự bộ phận $S$ có cận trên nhỏ nhất (cận dưới lớn nhất), thì cận đó là duy nhất.

    Có thể ký hiệu cận trên nhỏ nhất và cận dưới lớn nhất của $T$ lần lượt là $\sup T$ và $\inf T$. Nếu chính $S$, xét như tập con của nó, có cả phần tử lớn nhất và phần tử nhỏ nhất, thì $S$ được gọi là tập có thứ tự bị chặn.
...
Nếu một tập con của tập có thứ tự bộ phận có cận trên nhỏ nhất (cận dưới lớn nhất), thì cận đó là duy nhất. Tuy nhiên, điều này không áp dụng cho phần tử cực đại (cực tiểu). Ví dụ: xét tập có thứ tự bộ phận $S=\{\{0\},\{1\},\{2\},\{0,1\},\{0,2\},\{1,2\}\}$ với thứ tự bộ phận $\subseteq$ trên đó. Nó có $3$ phần tử cực đại và $3$ phần tử cực tiểu.

Có thể thêm một số điều kiện vào tập có thứ tự bộ phận để nếu phần tử cực đại (cực tiểu) tồn tại thì nó là duy nhất; nhờ đó có thể định nghĩa khái niệm phần tử lớn nhất (nhỏ nhất).

???+ note "Tập định hướng"
    Với tập tiền thứ tự không rỗng $S$ và tiền thứ tự $\preceq$ trên đó, nếu $(\forall~a,b\in S)~~(\exists~c\in S)~~a\preceq c\land b\preceq c$, thì $S$ được gọi là **tập định hướng** (directed set) hoặc **tập lọc** (filtered set) theo $\preceq$.

    Đôi khi tập $S$ thỏa mãn định nghĩa trên cũng được gọi là **tập định hướng lên** (upward directed set); tương tự, có thể định nghĩa **tập định hướng xuống** (downward directed set).

Tập định hướng cũng có thể được định nghĩa theo cách sau:

???+ note "Định nghĩa tương đương của tập định hướng"
    Với tập tiền thứ tự không rỗng $S$ và tiền thứ tự $\preceq$ trên đó, nếu mọi tập con hữu hạn $T$ của $S$ đều có cận trên, thì $S$ được gọi là tập định hướng theo $\preceq$.

Có các tính chất sau:

-   Nếu tập định hướng lên có phần tử cực đại, thì phần tử đó là duy nhất. Phần tử cực đại của tập định hướng lên được gọi là **phần tử lớn nhất** (greatest element).
-   Nếu tập định hướng xuống có phần tử cực tiểu, thì phần tử đó là duy nhất. Phần tử cực tiểu của tập định hướng xuống được gọi là **phần tử nhỏ nhất** (least element).

Trong một tập có thứ tự bộ phận có hướng, với mọi phần tử $a,b$, $\{a,b\}$ đều có cận trên. Nếu thay cận trên bằng cận trên nhỏ nhất, sẽ thu được định nghĩa của nửa dàn hợp.

Với tập có thứ tự bộ phận $S$ và thứ tự bộ phận $\preceq$ trên đó:

???+ note "Nửa dàn hợp"
    Nếu với mọi phần tử $a,b$ trong $S$, $\{a,b\}$ đều có cận trên nhỏ nhất $c$, thì $S$ được gọi là **nửa dàn hợp** (join-semilattice, upper semilattice), và $c$ được gọi là **hợp** (join) của $a$ và $b$, ký hiệu là $a\lor b$.

???+ note "Nửa dàn giao"
    Nếu với mọi phần tử $a,b$ trong $S$, $\{a,b\}$ đều có cận dưới lớn nhất $c$, thì $S$ được gọi là **nửa dàn giao** (meet-semilattice, lower semilattice), và $c$ được gọi là **giao** (meet) của $a$ và $b$, ký hiệu là $a\land b$.
...
Trong lý thuyết thứ tự, đối ngẫu là một khái niệm rất thường gặp, chẳng hạn các cặp đối ngẫu đã nhắc ở trên: phần tử cực đại và phần tử cực tiểu, cận trên và cận dưới, cận trên nhỏ nhất và cận dưới lớn nhất.

Với tập có thứ tự bộ phận $P$ và thứ tự bộ phận $\preceq_P$ trên đó, định nghĩa tập có thứ tự bộ phận **đối ngẫu** (dual, opposite) $P^d$ của nó như sau: $x\preceq_{P^d} y$ khi và chỉ khi $y\preceq_P x$. Đảo chiều các cạnh trong biểu đồ Hasse của $P$ sẽ thu được biểu đồ Hasse của $P^d$.

<span id="định-lý-dilworth-và-định-lý-mirsky"></span>
## Định lý Dilworth và định lý Mirsky

Với tập có thứ tự bộ phận hữu hạn $S$ và thứ tự bộ phận $\preceq$ trên đó, có cặp định lý đối ngẫu sau:

???+ note "Định lý Dilworth"
    Độ rộng của $S$ (kích thước của phản xích lớn nhất) bằng số xích ít nhất cần để phủ $S$.

    ??? note "Chứng minh"
        Xét quy nạp toán học. Khi $|S|\leq 3$, mệnh đề đúng trực tiếp.

        Giả sử mệnh đề đúng với mọi tập có thứ tự bộ phận có số phần tử nhỏ hơn $|S|$. Gọi độ rộng của $S$ là $d$. Nếu mọi phần tử trong $S$ đều đôi một không so sánh được, mệnh đề đúng trực tiếp; nếu không, lấy trong $S$ một xích có độ dài lớn hơn $1$, gọi phần tử nhỏ nhất của xích đó là $m$ và phần tử lớn nhất là $M$.

        Đặt $T=S\setminus\{m,M\}$. Nếu độ rộng của $T$ không vượt quá $d-1$, thì theo giả thiết quy nạp, $T$ có thể được phủ bởi nhiều nhất $d-1$ xích; do đó $S$ có thể được phủ bởi các xích này cộng thêm xích $\{m,M\}$, và mệnh đề đúng. Ngược lại, độ rộng của $T$ cũng là $d$; gọi $A$ là một phản xích có kích thước lớn nhất trong $T$.
...
Có thể dùng định lý Dilworth để chứng minh định lý sau:
...
???+ note "Định lý Erdős-Szekeres"
    ...
    ??? note "Chứng minh"
        ...
        Nếu độ rộng của tập có thứ tự bộ phận này không vượt quá $s$, thì theo định lý Dilworth, tập có thứ tự bộ phận này có thể được phủ bởi nhiều nhất $s$ xích. Nếu độ dài của mỗi xích đều không vượt quá $r$, thì số phần tử của dãy nhiều nhất là $rs$, mâu thuẫn với điều kiện. Do đó tồn tại một xích có độ dài ít nhất $r+1$, tương ứng với một dãy con không giảm độ dài $r+1$.

        Nếu độ rộng lớn hơn $s$, tồn tại một phản xích kích thước $s+1$. Sắp các phần tử của phản xích này theo chỉ số tăng dần sẽ thu được một dãy con giảm, nên cũng là dãy con không tăng, có độ dài $s+1$.
...
???+ note "[Luogu P1020 \[NOIP1999 Nhóm nâng cao\] Chặn tên lửa](https://www.luogu.com.cn/problem/P1020)"
    ...
    Nhập lần lượt độ cao của các tên lửa bay tới. Tính số tên lửa nhiều nhất mà một bộ hệ thống này có thể đánh chặn, và nếu muốn đánh chặn tất cả tên lửa thì cần trang bị ít nhất bao nhiêu bộ hệ thống như vậy.
    ...
    ??? note "Lời giải"
        ...
        Theo định lý Dilworth, có: **số dãy con không tăng ít nhất cần để phủ dãy bằng độ dài dãy con tăng dài nhất**. Vì vậy có thể giải bài này bằng biến thể $O(n\log n)$ của thuật toán LIS.
...
???+ note "[\[TJOI2015\] Toán tổ hợp](https://www.luogu.com.cn/problem/P3974)"
    ...
    ??? note "Lời giải"
        Không xét trọng số đỉnh của lưới, việc đi trên lưới theo quy tắc đã cho tương đương với đi trong một tập có thứ tự bộ phận có biểu đồ Hasse tương ứng là một DAG. Theo định lý Dilworth, **số xích ít nhất cần để phủ poset bằng kích thước phản xích lớn nhất**.
...
Trong các [thuật toán và cấu trúc dữ liệu cần dùng so sánh](https://en.cppreference.com/w/cpp/named_req/Compare#Standard_library) của C++ STL có các ứng dụng của lý thuyết thứ tự. Khi cần tự định nghĩa bộ so sánh trong C++, STL [yêu cầu](https://en.cppreference.com/w/cpp/named_req/Compare) bộ so sánh đó phải là **thứ tự yếu nghiêm ngặt**. Gọi $<$ là bộ so sánh tự định nghĩa, có thể định nghĩa:
...
10. [Một điều cần biết về hàm so sánh: thứ tự yếu nghiêm ngặt](https://codeforces.com/blog/entry/72525)
11. [Định lý Dilworth - Wikipedia](https://en.wikipedia.org/wiki/Dilworth%27s_theorem)
12. [Định lý Dilworth | Brilliant Math & Science Wiki](https://brilliant.org/wiki/dilworths-theorem/)
13. [Định lý hôn nhân Hall - Wikipedia](https://en.wikipedia.org/wiki/Hall's_marriage_theorem)
14. [Định lý hôn nhân Hall | Brilliant Math & Science Wiki](https://brilliant.org/wiki/hall-marriage-theorem/)
