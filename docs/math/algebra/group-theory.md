...
Không thể hiểu đại số trừu tượng nếu tách rời các ví dụ. Để minh họa cho các khái niệm bên dưới, xét nhóm đối xứng $D_6$ của tam giác đều.
...
???+ example "Ví dụ: nhóm đối xứng $D_6$ của tam giác đều (tiếp)"
    Trong nhóm $D_6$, chọn phép quay $r$ rồi lặp lại nó và phép nghịch đảo của nó sẽ thu được nhóm con $\{e,r,r^2\}$. Nhóm con này có thể viết là $\langle r\rangle$. Mỗi nhóm con không tầm thường của $D_6$ đều có thể được sinh ra bằng cách chọn một phép biến đổi nào đó.
...
Với một nhóm con cho trước, có thể định nghĩa lớp kề của nó.
...
Nhóm con chuẩn tắc là một lớp nhóm con rất quan trọng, một lý do là có thể định nghĩa nhóm thương dựa trên nó.
...
Để so sánh hai nhóm, cần xây dựng một ánh xạ giữa chúng. Những ánh xạ này không thể tùy ý; chúng phải bảo toàn cấu trúc nhóm, tức bảo toàn phép toán của nhóm trước và sau khi ánh xạ. Các ánh xạ như vậy được gọi là đồng cấu nhóm.
...
???+ info "Ký hiệu"
    Trong phần sau, nếu không gây nhầm lẫn, không phân biệt ký hiệu phép toán trong các nhóm $G$ và $H$, và sẽ lược bỏ các ký hiệu phép toán để trình bày ngắn gọn.
...
Với đồng cấu tổng quát, có thể tiếp tục hỏi bao nhiêu thông tin cấu trúc nhóm bị mất qua đồng cấu. Tiếp tục dùng ký hiệu ở trên. Đã biết $\varphi(G)$ là nhóm con của $H$, vấn đề then chốt là quan hệ giữa $\varphi(G)$ và $G$.
...
???+ example "Ví dụ: nhóm đối xứng $D_6$ của tam giác đều (tiếp)"
    ...
    Có thể kiểm tra $\varphi$ là đồng cấu nhóm; nó là toàn ánh nhưng không đơn ánh. Ý nghĩa của nó trực quan: mỗi phép đối xứng của nhóm được ánh xạ đến việc phép đó có lật hay không. Thông tin của nhóm $D_6$ bị nén bởi đồng cấu này chính là thông tin về góc quay. Chẳng hạn, nếu không lật, mọi góc quay đều được ánh xạ đến phần tử đơn vị của nhóm $C_2$.
...
??? note "Chứng minh"
    Trước hết, $N=\ker\varphi$ là nhóm con chuẩn tắc, vì với mọi $h\in N$ có $\varphi(ghg^{-1})=\varphi(g)\varphi(h)\varphi(g)^{-1}=\varphi(g)\varphi(g)^{-1}=e$, tức $ghg^{-1}\in\ker\varphi$. Sau đó xét ánh xạ $\Phi:G/N\rightarrow\varphi(G)$ thỏa $\Phi(gN)=\varphi(g)$. Ánh xạ này xác định tốt, vì nếu $g_1N=g_2N$ thì $g_1^{-1}g_2\in N$, suy ra $\varphi(g_1^{-1}g_2)=e$, tức $\varphi(g_1)=\varphi(g_2)$. Ánh xạ $\Phi$ là toàn ánh theo định nghĩa; nó cũng đơn ánh vì $\ker\Phi=\{gN:\varphi(g)=e\}=\{N\}$. Vì vậy $\Phi$ là đẳng cấu nhóm. Cuối cùng, $\varphi(g_1)\varphi(g_2)^{-1}=\varphi(g_1g_2^{-1})\in\varphi(G)$, nên theo tiêu chuẩn nhận biết nhóm con, $\varphi(G)$ là nhóm con.
...
Trong biểu đồ này, mọi mũi tên đều là đồng cấu nhóm, $N=\ker\varphi$ là hạt nhân của đồng cấu $\varphi$, và $\varphi(G)$ là ảnh của đồng cấu $\varphi$. Các ánh xạ lần lượt là: $\pi:g\mapsto gN$ là ánh xạ tự nhiên (toàn đồng cấu) từ nhóm $G$ đến nhóm thương $G/N$; $\Phi:gN\mapsto\varphi(g)$ là đẳng cấu; $\iota$ là phép nhúng (đơn đồng cấu). Biểu đồ giao hoán có nghĩa là hợp thành các ánh xạ trên hai đường đi khác nhau từ $G$ đến $H$ cho cùng kết quả, tức $\varphi=\iota\circ\Phi\circ\pi$. Biểu đồ này cho thấy rõ thông tin mà đồng cấu $\varphi$ làm mất được phản ánh trong $\pi$ và $\iota$.

### Các định lý đẳng cấu của nhóm

Công cụ mạnh để hiểu cấu trúc nhóm là các định lý đẳng cấu của nhóm. Phần trên đã có định lý đẳng cấu thứ nhất. Để nội dung đầy đủ, phần này nêu thêm các định lý đẳng cấu phổ biến khác.
...
??? note "Chứng minh"
    Nếu tích $AB$ là nhóm con, thì với mọi $a\in A$ và $b\in B$ có $ba=(a^{-1}b^{-1})^{-1}\in AB$, do đó $BA\subseteq AB$. Ngược lại, nếu $AB=BA$, thì với mọi $a_1,a_2\in A$ và $b_1,b_2\in B$ có $(a_1b_1)(a_2b_2)^{-1}=a_1b_1b_2^{-1}a_2^{-1}\in a_1BA=a_1AB=AB$, nên theo tiêu chuẩn nhận biết nhóm con, $AB$ là nhóm con.
...
???+ note "Định lý đẳng cấu thứ hai"
    Cho nhóm $G$ và các nhóm con $A,B\le G$ thỏa $A\le N_G(B)$. Khi đó $AB\le G$, $B\trianglelefteq AB$, $A\cap B\trianglelefteq A$, và $AB/B\cong A/(A\cap B)$. Trong phát biểu này, $N_G(B)$ là [bộ chuẩn hóa](#bo-chuan-hoa-va-bo-trung-tam-hoa) của $B$. Đặc biệt, một điều kiện đủ cho $A\le N_G(B)$ là $B\trianglelefteq G$.
...
??? info "Về nội dung các định lý đẳng cấu"
    Trong các giáo trình khác nhau, nội dung và tên gọi của các định lý đẳng cấu nhóm có thể khác nhau. Tài liệu này chọn một phiên bản phổ biến. [Wikipedia](https://en.wikipedia.org/wiki/Isomorphism_theorems#Note_on_numbers_and_names) có tóm tắt sự khác nhau về nội dung và tên gọi trong các giáo trình thông dụng.

## Tác động nhóm

Cách thứ ba để hiểu cấu trúc của một nhóm cho trước là xét tác động của nhóm lên một tập hợp.

Chẳng hạn, nhóm đối xứng của tam giác đều trong bài này được định nghĩa thông qua tác động của các phần tử nhóm (tức các phép đối xứng) lên tam giác. Một ví dụ khác, nhóm đối xứng $S_M$ có thể được định nghĩa thông qua tác động của các phần tử của nó lên tập $M$. Tác động trong ngữ cảnh này có nghĩa là mỗi phần tử của nhóm tương ứng với một hoán vị trên tập hợp.
...
???+ info "Ký hiệu"
    Trong phần sau, để trình bày gọn hơn, ký hiệu $\cdot$ trong tác động nhóm sẽ được lược bỏ.

### Quỹ đạo

Tác động nhóm là ánh xạ hai ngôi. Nếu cố định phần tử $g$ của nhóm, thu được một hoán vị trên tập hợp. Nếu cố định phần tử $x$ của tập hợp, thu được tất cả kết quả có thể của tác động của nhóm lên phần tử đó.
...
Dùng định lý Lagrange, có thể liên hệ độ dài quỹ đạo với số lớp kề của bộ ổn định. Đây là **định lý quỹ đạo-bộ ổn định** (orbit-stabilizer theorem).
...
???+ abstract "Lớp liên hợp"
    Cho nhóm $G$ và $g\in G$. **Lớp liên hợp** (conjugacy class) của phần tử $g$ trong $G$ là quỹ đạo của $g$ dưới tác động liên hợp. Nếu hai phần tử $g$ và $h$ nằm trong cùng một lớp liên hợp, gọi $g$ và $h$ là **liên hợp** (conjugate).
...
Nhóm dưới tác động liên hợp được chia thành nhiều lớp liên hợp. Vì thế, có thể viết **phương trình lớp** (class equation).
...
Dưới tác động liên hợp này, cũng có thể định nghĩa lớp liên hợp của một tập con của nhóm. Hạt nhân của nó vẫn là tâm của nhóm. Bộ ổn định của nó được gọi là bộ chuẩn hóa.
...
Phân tích sâu hơn tác động liên hợp của nhóm hữu hạn sẽ thu được định lý Sylow. Đây là công cụ mạnh để xử lý cấu trúc nhóm hữu hạn, và có thể nhanh chóng suy ra cấu trúc của nhiều nhóm cấp nhỏ.
...
Sau khi nắm các công cụ cơ bản để phân tích cấu trúc nhóm, phần này tập trung vào cấu trúc của một lớp nhóm.
...
Ý tưởng cơ bản của tích trực tiếp là: cho hai nhóm $G$ và $H$, xét tích Descartes $G\times H$; phép toán trên cặp $(g,h)$ được định nghĩa bằng cách thực hiện phép toán riêng trên từng thành phần, hai thành phần không ảnh hưởng lẫn nhau. Kết quả thu được là một nhóm lớn hơn, và hai nhóm ban đầu nhúng vào nhóm mới một cách tầm thường.
...
??? note "Chứng minh"
    Chứng minh dưới đây có hình thức đơn giản[^proof-abelian]; chứng minh sâu sắc hơn nên tham khảo định lý cấu trúc của môđun hữu hạn sinh trên miền lý tưởng chính[^module-pid]. Trong chứng minh, để viết gọn, dùng ký hiệu cộng thay cho ký hiệu nhân trong nhóm tổng quát; khi đó $0$ biểu diễn phần tử đơn vị, và ký hiệu $mx$ biểu diễn lũy thừa bậc $m$ của $x$.
    
    Giả sử $G$ có thể được sinh bởi ít nhất $k$ phần tử. Chứng minh định lý bằng quy nạp theo $k$. Khi $k=1$, kết luận suy ra ngay. Khi $k>1$, trong tất cả các bộ sinh $\langle x_1,x_2,\cdots,x_k\rangle$ của $G$, chọn bộ sao cho $x_1$ có cấp nhỏ nhất. Sẽ chứng minh $G=\langle x_1\rangle\times\langle x_2,\cdots,x_k\rangle$; theo giả thuyết quy nạp, nhóm sau đã phân tích được thành tích trực tiếp của $(k-1)$ nhóm vòng, nên bước quy nạp được chứng minh.
...
??? note "Chứng minh"
    Gọi $x$ và $y$ lần lượt là các phần tử sinh của $C_m$ và $C_n$. Vì $(m,n)=1$, có $|(x,y)|=mn=|C_m\times C_n|$. Do đó $(x,y)$ là phần tử sinh của $C_m\times C_n$, tức $C_m\times C_n=\langle(x,y)\rangle$. Nhóm này là nhóm vòng cấp $mn$, nên tất yếu đẳng cấu với $C_{mn}$.
