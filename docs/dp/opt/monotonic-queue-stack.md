...
???+ note "Lưu ý"
    -   Khi cần lấy giá trị nhỏ nhất, duy trì hàng đợi/ngăn xếp **tăng dần/không giảm**; chiều ngược lại tương tự.
...
-   Thêm các phần tử cần thiết: liên tục thêm phần tử vào hàng đợi đơn điệu cho tới khi phần tử hiện tại đạt biên phải của đoạn cần xét, sao cho mọi phần tử cần dùng đều nằm trong hàng đợi.
-   Loại bỏ đầu hàng đợi vượt biên: hàng đợi đơn điệu vốn duy trì cực trị của mọi phần tử đã chèn, nhưng bài toán thường yêu cầu cực trị trên một đoạn. Vì vậy, cần loại bỏ các phần tử nằm ngoài biên trái để mọi phần tử trong hàng đợi thuộc đoạn đang xét.
...
???+ note "Mô tả bài toán"
    Có $n$ loại vật phẩm; mỗi loại có trọng lượng $w_i$, giá trị $v_i$, số lượng $k_i$. Có một ba lô với giới hạn tải trọng $W$. Cần chọn các vật phẩm sao cho không vượt quá giới hạn trọng lượng và tổng giá trị lớn nhất. Tìm giá trị lớn nhất đó.

Nếu chưa biết DP ba lô, có thể tham khảo trước [DP ba lô](../knapsack.md). Đặt $f_{i,j}$ là giá trị lớn nhất khi xét $i$ loại vật phẩm đầu tiên và ba lô có tải trọng $j$. Công thức chuyển đơn giản là:
...
Như vậy bài toán được chuyển thành dạng tối ưu kinh điển bằng hàng đợi đơn điệu. $G_{x,y}$ có thể tính trong $O(1)$, nên với $y$ cố định, mọi $g_{x,y}$ có thể được tính trong $O\left(\left\lfloor\dfrac{W}{w_i}\right\rfloor\right)$. Do đó độ phức tạp để tính tất cả $g_{x,y}$ là $O\left(\left\lfloor\dfrac{W}{w_i}\right\rfloor\right)\times O(w_i)=O(W)$. Tổng độ phức tạp chuyển trạng thái giảm xuống $O(nW)$.

Khi cài đặt, cần duyệt $y$ trước để khi duyệt $x$ có thể dùng hàng đợi đơn điệu tối ưu. Hàng đợi đơn điệu lưu $x-k$, không lưu $k$, nên khi dùng cần lấy $G_{x-k,y}$ tương ứng bằng `f[last][q.front() * w[i] + y] - q.front() * v[i]`. Do $x-k\in[x-k_i,x]$, khi duyệt $x$ cần xóa khỏi hàng đợi các phần tử không nằm trong khoảng này.
...
Đặt $f_{i,j}$ là điểm vui vẻ lớn nhất có thể đạt được khi pháo hoa thứ $i$ được bắn và người chơi đang ở vị trí $j$.

Phương trình chuyển trạng thái là $f_{i,j}=\max\{f_{i-1,k}+b_i-|a_i-j|\}$, trong đó $j-(t_i-t_{i-1})\times d\le k\le j+(t_i-t_{i-1})\times d$.

Thử biến đổi:

Vì trong $\max$ có hằng số xác định $b_i$, có thể đưa hằng số đó ra ngoài.
...
Tiếp theo xét tối ưu bằng hàng đợi đơn điệu. Vì $\max$ trong công thức cuối chỉ liên quan tới giá trị lớn nhất của một đoạn liên tiếp trong trạng thái trước, khi tính trạng thái cho một $i$ mới, chỉ cần xây dựng và duy trì hàng đợi đơn điệu từ $f_{i-1}$, để tính $\max\{f_{i-1,k}\}$ trong thời gian khấu hao $O(1)$, rồi dùng công thức tính $f_{i,j}$.
