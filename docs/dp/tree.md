...
Với mỗi trạng thái có hai kiểu quyết định (trong đó các $x$ bên dưới đều là con của $i$):
...
Ta có thể dùng DFS và cập nhật lời giải tối ưu của nút hiện tại khi quay về tầng trên.

```cpp
--8<-- "docs/dp/code/tree/tree_1.cpp"
```

Thông thường, trạng thái của DP trên cây là lời giải tối ưu tại nút hiện tại. Trước hết dùng DFS duyệt lời giải tối ưu của mọi cây con, sau đó truyền lên nút cha để chuyển trạng thái. Cuối cùng, giá trị tại gốc chính là lời giải tối ưu cần tìm.
...
???+ note "Bài ví dụ [Luogu P2014 CTSC1997 Chọn môn](https://www.luogu.com.cn/problem/P2014)"
    ...
    Một học sinh muốn học $m$ môn. Hãy tìm số tín chỉ lớn nhất có thể đạt được.
    
    $n,m \leq 300$

Đặc điểm mỗi môn nhiều nhất chỉ có một môn tiên quyết tương tự với đặc điểm mỗi nút trong cây có gốc nhiều nhất chỉ có một nút cha.

Vì vậy có thể dựng cây dựa trên tính chất này, và toàn bộ các môn học tạo thành một rừng. Để thuận tiện, ta thêm một môn học có $0$ tín chỉ (đánh số là $0$) làm môn tiên quyết của mọi môn không có tiên quyết; khi đó rừng được biến thành một cây có gốc là môn $0$.

Đặt $f(u,i,j)$ là số tín chỉ lớn nhất trong cây con gốc $u$ khi đã duyệt $i$ cây con đầu tiên của nút $u$ và chọn $j$ môn.

Quá trình chuyển trạng thái kết hợp đặc điểm của DP trên cây và [DP ba lô](./knapsack.md). Ta duyệt từng con $v$ của $u$, đồng thời duyệt số môn được chọn trong cây con gốc $v$, rồi gộp kết quả của cây con vào $u$.

Gọi số con của nút $x$ là $s_x$, kích thước cây con gốc $x$ là $\textit{siz}_x$, ta có phương trình chuyển trạng thái:

$$
f(u,i,j)=\max_{v,k \leq j,k \leq \textit{siz}_v} f(u,i-1,j-k)+f(v,s_v,k)
$$

Chú ý các điều kiện ràng buộc trong phương trình trên; chúng bảo đảm rằng một số trạng thái vô nghĩa sẽ không bị truy cập.

Chiều thứ hai của $f$ có thể dễ dàng được lược bỏ bằng mảng cuộn. Khi đó cần chú ý duyệt $j$ theo thứ tự giảm.
...
Tiếp theo, ta dùng một số ví dụ để làm quen với nội dung này.

???+ note "Bài ví dụ [\[POI2008\] STA-Station](https://www.luogu.com.cn/problem/P3478)"
    Cho một cây có $n$ nút. Hãy tìm một nút sao cho khi lấy nút đó làm gốc, tổng độ sâu của mọi nút là lớn nhất.

Gọi $u$ là nút hiện tại, $v$ là một nút con của nó. Trước hết dùng $s_i$ để biểu diễn số nút trong cây con gốc $i$, khi đó $s_u=1+\sum s_v$. Rõ ràng cần một lần DFS để tính mọi $s_i$; lần DFS này chính là tiền xử lý, giúp ta biết tổng số nút trong cây con khi lấy một nút nào đó làm gốc.
...
Vì vậy, trong lần DFS thứ hai, duyệt toàn bộ cây và chuyển trạng thái theo $f_v=f_u+n-2\times s_v$, ta có thể tính tổng độ sâu khi lấy từng nút làm gốc. Cuối cùng chỉ cần duyệt một lần các tổng độ sâu này để lấy đáp án.
